# Instalacion del Facturador Smart

Este repositorio contiene un script de instalacion que automatiza la instalacion y configuracion del **Facturador Smart**. Con este script se realiza lo siguiente:

- Actualiza el sistema e instala dependencias básicas.
- Instala **git**, **Docker** y **Docker Compose**.
- Configura **Let's Encrypt** para certificados SSL (opcional).
- Configura un proxy Nginx mediante Docker.
- Clona el repositorio del Facturador Smart desde GitLab.
- Configura el entorno de la aplicacion, generando credenciales aleatorias para MySQL y definiendo variables de entorno.
- Levanta los servicios con Docker Compose y ejecuta comandos de Artisan para migraciones, seeding, generación de claves y otros ajustes.
- Configura permisos y Supervisor para la ejecucion de procesos.
...
1. Abre el archivo `install.sh` en tu editor preferido:
   ```bash
   nano install.sh
...
3. Ejecuta el script pasando como parametro el dominio que deseas configurar. Opcionalmente, puedes pasar el número de servicio y el puerto para MySQL (por defecto es 3306):
   ```bash
   ./install.sh dominio.com 1 3306
...
- **HOST:** Dominio para la configuración (obligatorio).
- **SERVICE_NUMBER:** Número de servicio (por defecto `'1'`).
- **MYSQL_PORT_HOST:** Puerto de MySQL en el host (por defecto `'3306'`).

Se generan variables para el nombre del directorio del proyecto y credenciales aleatorias para MySQL.

### Actualización e Instalación de Dependencias
- Actualiza el sistema.
- Instala **git**, **Docker**, **Docker Compose** y **Let’s Encrypt**.
- Crea la red y configura el proxy Nginx mediante Docker.

### Clonación y Configuración del Proyecto
- Clona el repositorio del Facturador Smart desde GitLab.
- Copia archivos de configuración (por ejemplo, `supervisor.conf.example`) y genera archivos para Docker Compose y para la configuración de Nginx.
- Actualiza el archivo `.env` con los datos generados y la URL base.

### Levantamiento de Servicios y Ajustes
- Levanta los contenedores con `docker-compose up -d`.
- Ejecuta comandos dentro de los contenedores para:
  - Instalar dependencias PHP.
  - Migrar la base de datos.
  - Generar la llave de la aplicación.
  - Enlazar storage, entre otros.
- Configura **Supervisor** para iniciar los procesos necesarios.

### Instalación de SSL (Opcional)
- Pregunta al usuario si desea instalar SSL gratuito mediante **Let’s Encrypt**.
- Si se acepta, se procede a generar el certificado y se actualizan las configuraciones correspondientes en el archivo `.env`.

### Información Final
- Al terminar, se muestran en pantalla:
  - Las credenciales generadas.
  - La URL de acceso al proyecto.
  - Datos de acceso remoto a MySQL.
