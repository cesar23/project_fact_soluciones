...
class AccountController extends Controller
{
    public function index()
    {
        ...
        return view('account::accounting.index', compact('companies'));
    }
    ...
    public function download(Request $request)
    {
        ...
        $with_nv = $request->input('with_nv') == "true";
        $with_expenses = $request->input('with_expenses');
        ...
        $records = $this->getDocuments($d_start, $d_end, $type_operation, $with_nv);
        if(($type === 'siscont' || $type === 'contasis') && $with_nv && $type_operation == 'sale'){
            $nv = $this->getDocumentsWithNv($d_start, $d_end);
            $records = collect($records)->merge($nv);
        }
        
        if($type_operation == 'purchase' && $type === 'contasis' && $with_expenses){
            $expenses = $this->getExpenses($d_start, $d_end);
            $records = collect($records)->merge($expenses);
        }
        $description_operation = ($type_operation === 'sale') ? '_Ventas' : '_Compras';
        $filename = 'Reporte_' . ucfirst($type) . $description_operation . '_' . date('YmdHis');
        ...
        switch ($type) {
            ...
            case 'contasis':
                ...
                $data = [
                    'records' => $this->getStructureContasis($records, $type_operation),
                    'company' => $company,
    ...
    private function getDocuments($d_start, $d_end, $type_operation = 'sale', $with_nv = false)
    {
    ...
    private function getExpenses($d_start, $d_end){
    ...
    private function getDocumentsWithNv($d_start, $d_end)
    {
    ...
    private function getStructureContasis($documents, $type_operation = 'sale')
    {
        

        return $documents->transform(function ($row) use ($type_operation) {
            $company_account = CompanyAccount::first();
            ...
            $payment_condition = $row->payment_condition_id == '01' ? 'CON' : 'CRED';

            $document_type_id = "01";
            if($row->document_type_id == null || $row->document_type_id == 'NE76'){
                $document_type_id = "12";
            }else{
                $document_type_id = ($row->document_type_id === '01') ? '01' : '03';
            }
        
            if($type_operation === 'purchase'){
                if(isset($row->supplier)){
                    if(is_object($row->supplier)){
                        $customer_identity_document_type_id = $row->supplier->identity_document_type_id;
                        $customer_number = $row->supplier->number;
                        $customer_name = $row->supplier->name;
                    }else if(is_array($row->supplier)){
                        $customer_identity_document_type_id = $row->supplier['identity_document_type_id'];
                        $customer_number = $row->supplier['number'];
                        $customer_name = $row->supplier['name'];
                    }else{
            ...
            if($type_operation === 'purchase' && $document_type_id === '12'){
                if(strpos($row->number, '-') !== false) {
                    $parts = explode('-', $row->number);
                    $series = '00' . $parts[0];
                    $number = str_pad($parts[1], 13, '0', STR_PAD_LEFT);
                }
            }
            return [
                'date_of_issue' => $row->date_of_issue->format('d/m/Y'),
                ...
                'series' => $series,
                'number' => $number,
                'customer_identity_document_type_id' => $customer_identity_document_type_id,
