...
class ReportKardexCollection extends ResourceCollection
{
    ...
    public static function determinateRow($row)
    {
        ...
        switch ($row->inventory_kardexable_type) {
            ...
            case $models[0]: //venta
                $sale_note_discounted_stock = isset($row->inventory_kardexable->sale_note_id) && !$configuration->emit_prepayment_document_from_sale_note;
                $cpe_input = ($row->quantity > 0) ?  (isset($sale_note_discounted_stock) || isset($row->inventory_kardexable->order_note_id) ? "-" : number_format($row->quantity,2)) : "-";
                $cpe_output = ($row->quantity < 0) ?  (isset($sale_note_discounted_stock) || isset($row->inventory_kardexable->order_note_id) ? "-" : number_format($row->quantity,2)) : "-";
                ...
                $doc_balance = ($sale_note_discounted_stock || isset($row->inventory_kardexable->order_note_id) || $cpe_discounted_stock) ? self::$balance += 0 : self::$balance += $row->quantity;
                ...
                return [
                    ...
                    'output' => $cpe_output,
                    'balance' => $doc_balance,
                    'sale_note_asoc' => $sale_note_discounted_stock ? optional($row->inventory_kardexable)->sale_note->number_full : "-",
