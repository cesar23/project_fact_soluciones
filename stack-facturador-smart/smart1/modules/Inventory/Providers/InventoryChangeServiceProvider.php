        ...
        Inventory::created(function ($inventory) {
            ...
            switch ($inventory->type) {
                ...
                case 2:
                    ...
                    $quantity = -1 * $inventory->quantity;
                    $this->updateStock($inventory->item_id, -1 * $inventory->quantity, $inventory->warehouse_id,true);
                    //Arrival
                    $this->createInventoryKardex($inventory, $inventory->item_id, $inventory->quantity, $inventory->warehouse_destination_id);
                    $this->updateStock($inventory->item_id, $inventory->quantity, $inventory->warehouse_destination_id,true);
                    break;
                case 3:
                    $this->createInventoryKardex($inventory, $inventory->item_id, -1 * $inventory->quantity, $inventory->warehouse_id);
                    $quantity = -1 * $inventory->quantity;
            ...
            (new ItemCostHistoryService)->insertItemMovement($inventory->item_id, $inventory->warehouse_id, $quantity, 0, $inventory->id, 'Modules\Inventory\Models\Inventory');
        });
    }
