...
trait InventoryTrait
{
    ...
    {
        $records = Establishment::whereActive()->get();
    ...
    public function optionsItemFull($search = null, $take = null)
    {
        ...
        if ($search) {
            ...
            $query->where('text_filter', 'like', "%{$search}%")
    ...
    {
        $records = User::get()->transform(function ($row) {
            return [
                'id' => $row->id,
                'name' => $row->name
            ];
    ...
    public function optionsWarehouse()
    {
        ...
        $records = Warehouse::where('active', true);
        if($warehouse_id){
            $records = $records->where('id', $warehouse_id);
        }   
        $records = $records->get();
        return collect($records)->transform(function ($row) {
            return [
    ...
    private function updateStock($item_id, $quantity, $warehouse_id, $is_transfer = false)
    {
        ...
        $w_stock = $item_warehouse->stock + $quantity;

        if ($quantity < 0 && $item_warehouse->item->unit_type_id !== 'ZZ') {
            $restrict_stock_quantity = $item_warehouse->restrict_stock_quantity;
            $stock_control = ($inventory_configuration->stock_control) && ($w_stock < 0);
            if ($restrict_stock_quantity > 0) {

                $stock_warehouse = $item_warehouse->stock;

                $absolute_quantity = abs($quantity);

                $stock_control = $absolute_quantity > ($stock_warehouse - $restrict_stock_quantity);
            }
     ...
     */
     ...
     private function updateDataSizes($document_item)
     {
            ...
            if ($document_item->item->IdLoteSelected != null) {
                ...
                if (is_array($document_item->item->IdLoteSelected)) {
                    ...
                    foreach ($lotesSelecteds as $item) {
                        ...
                        if (isset($item->compromise_quantity)) {
    ...
    public function itemHasItemSet($items)
    {
        $has_item_set = false;
        foreach ($items as $item) {
            if ($item['item']['is_set']) {
    ...
    public function checkPurchaseOrden(?string $purchaseOrden, ?int $excludeId = null, ?string $excludeModel = null,$items = [],?int $sale_note_id = null)
    {   

        if($excludeModel == Document::class && $sale_note_id){
            return;
        }
        
        if(!$purchaseOrden) return;
        $configuration = Configuration::first();
        $has_item_set = $this->itemHasItemSet($items);
        if (!$configuration || !$configuration->purchase_orden_in_item_set || !$has_item_set) {
            return;
        }

        $normalized = strtoupper(trim($purchaseOrden));
        $tenant_connection = DB::connection('tenant');

        // Consulta para documentos
        $queryDocuments = $tenant_connection->table('documents')
            ...
            ->where(function ($query) {
                ...
                $query->whereNotExists(function ($subquery) {
                    $subquery->select(DB::raw(1))
                        ->from('notes')
                        ->whereRaw('notes.affected_document_id = documents.id');
                })
                ->orWhereExists(function ($subquery) {
                    $subquery->select(DB::raw(1))
                        ->from('notes')
                        ->whereRaw('notes.affected_document_id = documents.id')
                        ->join('documents as doc', 'notes.document_id', '=', 'doc.id')
                        ->whereIn('doc.state_type_id', ['11', '13', '09']);
                });
            })
            ->select(DB::raw("'document' as type, id, series, number"))
            ->whereRaw('UPPER(TRIM(purchase_order)) = ?', [$normalized]);

        // Aplicar la exclusión solo si el modelo es 'documents'
        if ($excludeModel === Document::class && $excludeId) {
            $queryDocuments->where('id', '!=', $excludeId);
        }

        $querySaleNotes = $tenant_connection->table('sale_notes')
            ->whereNotIn('state_type_id', ['11', '13'])
            ->select(DB::raw("'sale_note' as type, id, series, number"))
            ->whereRaw('UPPER(TRIM(purchase_order)) = ?', [$normalized]);

        // Aplicar la exclusión solo si el modelo es 'sale_note'
        if ($excludeModel === SaleNote::class && $excludeId) {
            $querySaleNotes->where('id', '!=', $excludeId);
        }

        // Realizar la consulta con unión
        $result = $queryDocuments->unionAll($querySaleNotes)
            ->limit(1)
            ->first();

        if ($result) {
            $number_document = $result->series.'-'. $result->number;
            throw new Exception("El número de orden de compra '{$purchaseOrden}' ya existe en el sistema! (Documento: {$number_document})");
        }
    }
}
