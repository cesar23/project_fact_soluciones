<?php

    namespace Modules\Inventory\Models;

    use App\Models\Tenant\Item;
    use App\Models\Tenant\ModelTenant;
    use Carbon\Carbon;
    use Eloquent;
    use Hyn\Tenancy\Traits\UsesTenantConnection;
    use Illuminate\Database\Eloquent\Builder;

    /**
     * \Modules\Inventory\Models\ItemWarehouse
     *
     * @property int         $id
     * @property int         $item_id
     * @property int         $warehouse_id
     * @property float       $stock
     * @property Carbon|null $created_at
     * @property Carbon|null $updated_at
     * @property Item        $item
     * @property Warehouse   $warehouse
     * @method static Builder|ItemWarehouse newModelQuery()
     * @method static Builder|ItemWarehouse newQuery()
     * @method static Builder|ItemWarehouse query()
     * @method static Builder|ItemWarehouse whereWarehouse($warehouse_id)
     * @mixin ModelTenant
     * @mixin Eloquent
     */
    class ItemWarehouse extends ModelTenant
    {
        use UsesTenantConnection;

        protected $table = 'item_warehouse';

        protected $fillable = [
            'item_id',
            'warehouse_id',
            'stock',
            'restrict_stock_quantity',
        ];

        protected $casts = [
            'stock' => 'float',
            'item_id' => 'int',
            'warehouse_id' => 'int',
        ];

        /**
         * @return \Illuminate\Database\Eloquent\Relations\BelongsTo
         */
        public function warehouse()
        {
            return $this->belongsTo(Warehouse::class);
        }

        /**
         * @return \Illuminate\Database\Eloquent\Relations\BelongsTo
         */
        public function item()
        {
            return $this->belongsTo(Item::class);
        }


        /**
         * @param Builder $query
         * @param         $warehouse_id
         *
         * @return Builder
         */
        public function scopeWhereWarehouse(\Illuminate\Database\Eloquent\Builder $query, $warehouse_id)
        {
            if (!is_null($warehouse_id) && $warehouse_id !== 'all') {
                return $query->where('warehouse_id', $warehouse_id);
            }
            return $query;
        }

        
        /**
         * 
         * Filtrar por categoria del item - reporte inventario
         * 
         * @param Builder $query
         * @param int $category_id
         *
         * @return Builder
         */
        public function scopeWhereItemCategory(Builder $query, $category_id)
        {
            return $query->whereHas('item', function ($q) use($category_id){
                $q->where('category_id', $category_id);
            });
        }

        
        /**
         * 
         * Filtrar por marca del item - reporte inventario
         * 
         * @param Builder $query
         * @param int $brand_id
         *
         * @return Builder
         */
        public function scopeWhereItemBrand(Builder $query, $brand_id)
        {
            return $query->whereHas('item', function ($q) use($brand_id){
                $q->where('brand_id', $brand_id);
            });
        }


//    public function scopeWhereFilter($query, $filter, $stock_min)
//    {
//        if($filter === '02') {
//            return $query->where('stock', '<', 0);
//        }
//        if($filter === '03') {
//            return $query->where('stock', '=', 0);
//        }
//        if($filter === '04') {
//            return $query->where('stock', '>', 0)
//                         ->where('stock', '<=', $stock_min);
//        }
//        if($filter === '05') {
//            return $query->where('stock', '>', $stock_min);
//        }
//
//        return $query;
//    }


        /**
         *
         * Obtener stock del producto por almacen
         *
         * @param  Builder $query
         * @param  int $item_id
         * @param  int $warehouse_id
         * @return Builder
         */
        public function scopeGetItemStockData($query, $item_id, $warehouse_id)
        {
            return $query->where([
                        ['item_id', $item_id],
                        ['warehouse_id', $warehouse_id]
                    ]);
        }

        /**
         * Separar stock para transformación
         *
         * @param int $item_id
         * @param int $warehouse_id
         * @param float $quantity
         * @param int $order_transformation_id
         * @return bool
         */
        public static function separateStockForTransformation($item_id, $warehouse_id, $quantity, $order_transformation_id)
        {
            $itemWarehouse = self::where('item_id', $item_id)
                ->where('warehouse_id', $warehouse_id)
                ->first();

            if (!$itemWarehouse || $itemWarehouse->stock < $quantity) {
                return false;
            }

            // Restar del stock principal
            $itemWarehouse->stock -= $quantity;
            $itemWarehouse->save();

            // Crear registro en separated_stock
            \Modules\Preparation\Models\SeparatedStock::create([
                'item_id' => $item_id,
                'warehouse_id' => $warehouse_id,
                'order_transformation_id' => $order_transformation_id,
                'quantity' => $quantity,
                'description' => 'Stock separado para transformación'
            ]);

            return true;
        }

        /**
         * Restaurar stock separado
         *
         * @param int $order_transformation_id
         * @return bool
         */
        public static function restoreSeparatedStock($order_transformation_id)
        {
            $separatedStocks = \Modules\Preparation\Models\SeparatedStock::where('order_transformation_id', $order_transformation_id)->get();

            foreach ($separatedStocks as $separatedStock) {
                $itemWarehouse = self::where('item_id', $separatedStock->item_id)
                    ->where('warehouse_id', $separatedStock->warehouse_id)
                    ->first();

                if ($itemWarehouse) {
                    $itemWarehouse->stock += $separatedStock->quantity;
                    $itemWarehouse->save();
                }

                $separatedStock->delete();
            }

            return true;
        }

    }
