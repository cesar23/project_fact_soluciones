...
class OrderNote extends ModelTenant
{
    ...
    {
        return $this->prefix . '-' . (($this->number && $this->number !== '#') ? $this->number : $this->id);
    }
    ...
    {
        return $this->hasMany(PersonFoodDealer::class);
    }
    /**
     * @return BelongsTo
     */
    ...
    public function getCollectionData()
    {
        ...
        $quotation = $connection->table('quotations')
        ->select('id', 'number','prefix')
        ->find($this->quotation_id);
        if ($quotation !== null) {
            $quotation = [
                'id' => $quotation->id,
                'full_number' => $quotation->prefix.'-'.($quotation->number ?? $quotation->id),
            ];
        } else {
            $quotation = [];
        }
        ...
        if(BusinessTurn::isFoodDealer()){
            $person_food_dealer = $this->person_food_dealer()->first();
            $food_dealer_auth = $person_food_dealer->food_dealer_auth;
            if($food_dealer_auth){
                $auth_by = $food_dealer_auth->user_name;
            }
        }
        return [
            'auth_by' => $auth_by,
