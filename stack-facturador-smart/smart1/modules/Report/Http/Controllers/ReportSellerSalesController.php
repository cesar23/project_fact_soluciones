...
use App\Exports\SellerSalesExportConsolidated;
...
class ReportSellerSalesController extends Controller
{
    protected $d_start;
    protected $d_end;
    ...
    public function index()
    {
        ...
        return view('report::seller_sales.index');
    }
    ...
    public function records(Request $request)
    {
        ...
        $records = $this->getSellerRecords($request);
    ...
    private  function getSellerRecords(Request $request)
    {
        ...
        $records = $records->join('users', 'documents.seller_id', '=', 'users.id')
            ...
            ->whereNotIn('documents.state_type_id', ['11', '09']);
        ...
        $saleNotesQuery->whereNotIn('sale_notes.state_type_id', ['11', '09']);
    ...
    public function pdf(Request $request)
    {
        ...
        $records = $this->getSellerRecords($request)->orderBy('seller_name')
        ->get()
        ->groupBy(['seller_name']);
        $d_start = $this->d_start;
        $d_end = $this->d_end;
        $pdf = PDF::loadView('report::seller_sales.report_pdf', compact("records", "company", "d_end", "d_start"))->setPaper('a4', 'landscape');
    ...
    public function excel(Request $request)
    {
        ...
        // $filters = $request->all();
        $summary_sales_export = new SellerSalesExport();
    ...
    public function excelConsolidated(Request $request)
    {
        ...
        $records = $this->getSellerRecords($request)->orderBy('seller_name')
            ...
            ->groupBy(['seller_name']);
        // $filters = $request->all();
        $summary_sales_export = new SellerSalesExportConsolidated();
        $summary_sales_export
            ->records($records)
            ->company($company)
            ->d_start($this->d_start)
        ...
        return $summary_sales_export->download('Reporte_Ventas_Vendedor_Consolidado_' . Carbon::now() . '.xlsx');
    }
}
