...
class ReportOrderNoteConsolidatedController extends Controller
{
    ...
    public function pdf(Request $request)
    {
        ...
        $records = $this->getRecordsOrderNotes($request->all(), OrderNoteItem::class)->get()->sortBy(function ($row) {
    ...
    public function pdfTicketsTotal(Request $request)
    {
        ...
        $params = $request->all();
        $pdf = PDF::loadView(
            'report::order_notes_consolidated.report_pdf_totals_58',
            compact('records', 'company', 'establishment', 'params')
        )->setPaper([0, 0, 219.21, 500], 'portrait');
        $filename = 'Reporte_Consolidado_Items_Pedidos_Totales_' . date('YmdHis');
    ...
    public function pdfTicketsTotal80(Request $request)
    {
        ...
        $params = $request->all();
        $pdf = PDF::loadView(
            'report::order_notes_consolidated.report_pdf_totals_80',
            compact('records', 'company', 'establishment', 'params')
        )->setPaper([0, 0, 219.21, 500], 'portrait');
        $filename = 'Reporte_Consolidado_Items_Pedidos_Totales_' . date('YmdHis');
        return $pdf->stream($filename . '.pdf');
