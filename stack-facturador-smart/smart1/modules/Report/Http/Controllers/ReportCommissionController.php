...
use Modules\Report\Exports\CommissionExport2;
use Modules\Report\Exports\CommissionExportV2;
...
class ReportCommissionController extends Controller
{
    ...
    public function excel_detailed_v2(Request $request){
        $company = Company::first();
        $establishment = ($request->establishment_id) ? Establishment::findOrFail($request->establishment_id) : auth()->user()->establishment;
        $records = $this->getRecords($request->all(), User::class)->get();
        return (new CommissionExportV2)
            ->records($records)
            ->company($company)
            ->establishment($establishment)
            ->request($request)
            ->download('Reporte_Comision_Vendedor_' . Carbon::now() . '.xlsx');
    }
    public function excel_detailed(Request $request)
    {
        $company = Company::first();
        $establishment = ($request->establishment_id) ? Establishment::findOrFail($request->establishment_id) : auth()->user()->establishment;
        $records = $this->getRecords($request->all(), User::class)->get();
        return (new CommissionExport2)
            ->records($records)
            ->company($company)
            ->establishment($establishment)
            ->request($request)
