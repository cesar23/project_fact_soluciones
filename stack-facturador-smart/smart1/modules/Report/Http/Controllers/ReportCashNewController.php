...
class ReportCashNewController extends Controller
{
    ...
    public function getCashClosuresByUserAndDate(Request $request)
    {
        ...
        foreach ($cash_ids as $id) {
            $current_data = $this->setDataToReportNew($id);
            if (!$combined_data) {
    ...
    private function combineAllData($combined_data, $current_data)
    {
    }
    ...
    private function setDataToReportNew($cash_id = 0)
    {
        ...
        // Agrupar payment_ids por tipo para consultas en lote
        $payment_ids_by_type = [
        ...
        foreach ($cash_documents as $cash_document) {
            $payment_ids_by_type[$cash_document->payment_type][] = $cash_document->payment_id;
        }

        // Inicializar métodos de pago
        $methods_payments = collect(PaymentMethodType::all())->transform(function ($row) {
            return (object)[
                'id' => $row->id,
        ...
        // Procesar DocumentPayments en lote
        if (!empty($payment_ids_by_type['App\Models\Tenant\DocumentPayment'])) {
            $document_payments = DocumentPayment::with([
                'document:id,total,currency_type_id,exchange_rate_sale,seller_id,state_type_id',
                'document.items:id,document_id,item_id,unit_price,quantity',
            ])
                ->whereIn('id', $payment_ids_by_type['App\Models\Tenant\DocumentPayment'])
                ->get();

            foreach ($document_payments as $document_payment) {
                if (isset($document_payment->document->seller_id)) {
                    $document = $document_payment->document;
                    if(!in_array($document->state_type_id, $this->state_types)){
                        continue;
                    }
                    $seller_id = $document_payment->document->seller_id;
                    ...
                    $percentage = $payment / $total;
                    $payment_method_type_id = $document_payment->payment_method_type_id;

                    if ($currency_type_id !== 'PEN') {
                        $payment = $payment * $document->exchange_rate_sale;
                    }

                    $sellers[$seller_id]['payment'] += $payment;
                    $cpe_amount += $payment;

                    // Calcular ganancia y comisión
                    $gain_items = $this->getGainByItems($document->items);

                    $document_gain = $gain_items['gain'] * $percentage;
                    $document_comission = $gain_items['comission'] * $percentage;
                    $sellers[$seller_id]['gain'] += $document_gain;
                    $sellers[$seller_id]['comission'] += $document_comission;

                    // Actualizar métodos de pago
                    foreach ($methods_payments as $record) {
        ...
        // Procesar SaleNotePayments en lote
        if (!empty($payment_ids_by_type['App\Models\Tenant\SaleNotePayment'])) {
            $sale_note_payments = SaleNotePayment::with([
                'sale_note:id,total,currency_type_id,exchange_rate_sale,seller_id,state_type_id',
                'sale_note.items:id,sale_note_id,item_id,unit_price,quantity',
            ])
                ->whereIn('id', $payment_ids_by_type['App\Models\Tenant\SaleNotePayment'])
                ->get();

            foreach ($sale_note_payments as $sale_note_payment) {
                if (isset($sale_note_payment->sale_note->seller_id)) {
                    $sale_note = $sale_note_payment->sale_note;
                    if(!in_array($sale_note->state_type_id, $this->state_types)){
                        continue;
                    }
                    $seller_id = $sale_note_payment->sale_note->seller_id;
                    if (!isset($sellers[$seller_id])) {
                        $sellers[$seller_id] = [
                            'payment' => 0,
                            'gain' => 0,
                    ...
                    $payment = $sale_note_payment->payment;
                    $percentage = $payment / $total;
                    $payment_method_type_id = $sale_note_payment->payment_method_type_id;

                    if ($currency_type_id !== 'PEN') {
                        $payment = $payment * $sale_note->exchange_rate_sale;
                    }

                    $sellers[$seller_id]['payment'] += $payment;
                    $sale_note_amount += $payment;

                    // Calcular ganancia y comisión

                    $gain_items = $this->getGainByItems($sale_note->items);
            

                    $sale_note_gain = $gain_items['gain'] * $percentage;
                    $sale_note_comission = $gain_items['comission'] * $percentage;
                    $sellers[$seller_id]['gain'] += $sale_note_gain;
                    $sellers[$seller_id]['comission'] += $sale_note_comission;
        ...
        // Procesar QuotationPayments en lote
        if (!empty($payment_ids_by_type['Modules\Sale\Models\QuotationPayment'])) {
            $quotation_payments = QuotationPayment::with([
                'quotation:id,total,currency_type_id,exchange_rate_sale,seller_id',
                'quotation.items:id,item_id,unit_price,quantity,item',
            ])
                ->whereIn('id', $payment_ids_by_type['Modules\Sale\Models\QuotationPayment'])
                ->get();

            foreach ($quotation_payments as $quotation_payment) {
                if (isset($quotation_payment->quotation->seller_id)) {
                    $seller_id = $quotation_payment->quotation->seller_id;
                    if (!isset($sellers[$seller_id])) {
                        $sellers[$seller_id] = [
                            'payment' => 0,
                            'gain' => 0,
                    ...
                    $percentage = $payment / $total;
                    $payment_method_type_id = $quotation_payment->payment_method_type_id;

                    if ($currency_type_id !== 'PEN') {
                        $payment = $payment * $quotation->exchange_rate_sale;
                    }

                    $sellers[$seller_id]['payment'] += $payment;

                    // Calcular ganancia y comisión
                    $gain_items = $this->getGainByItems($quotation->items);
                    $quotation_gain = $gain_items['gain'] * $percentage;
                    $quotation_comission = $gain_items['comission'] * $percentage;
                    $sellers[$seller_id]['gain'] += $quotation_gain;
                    $sellers[$seller_id]['comission'] += $quotation_comission;
        ...
        // Procesar PurchasePayments en lote (solo para egress)
        if (!empty($payment_ids_by_type['App\Models\Tenant\PurchasePayment'])) {
            $purchase_payments = PurchasePayment::with([
                'purchase:id,currency_type_id,exchange_rate_sale,seller_id,state_type_id'
            ])
                ->whereIn('id', $payment_ids_by_type['App\Models\Tenant\PurchasePayment'])
                ->get();

            foreach ($purchase_payments as $purchase_payment) {
                if (isset($purchase_payment->purchase->seller_id)) {
                    $purchase = $purchase_payment->purchase;
                    if(!in_array($purchase->state_type_id, $this->state_types)){
                        continue;
                    }
                    $seller_id = $purchase_payment->purchase->seller_id;
                    $currency_type_id = $purchase->currency_type_id;
                    $payment = $purchase_payment->payment;

                    if ($currency_type_id !== 'PEN') {
                        $payment = $payment * $purchase->exchange_rate_sale;
                    }
                    $payment_method_type_id = $purchase_payment->payment_method_type_id;

                    $egress_amount += $payment;
                    foreach ($methods_payments as $record) {
                        if ($record->id == $payment_method_type_id) {
                            if ($record->is_cash) {
                                $cash_egress += $payment;
        ...
        // Procesar ExpensePayments en lote (solo para egress)
        if (!empty($payment_ids_by_type['Modules\Expense\Models\ExpensePayment'])) {
            $expense_payments = ExpensePayment::with([
                'expense:id,currency_type_id,exchange_rate_sale'
            ])
                ->whereIn('id', $payment_ids_by_type['Modules\Expense\Models\ExpensePayment'])
                ->get();

            foreach ($expense_payments as $expense_payment) {
                $expense = $expense_payment->expense;
                $currency_type_id = $expense->currency_type_id;
                $payment = $expense_payment->payment;
                if ($currency_type_id !== 'PEN') {
                    $payment = $payment * $expense->exchange_rate_sale;
                }


                $payment_method_type_id = $expense_payment->payment_method_type_id;
                $egress_amount += $payment;
                foreach ($methods_payments as $record) {
                    if ($record->id == $payment_method_type_id) {
                        if ($record->is_cash) {
        ...
        $seller_keys = array_keys($sellers);
        $users = User::select('id', 'name')
            ->whereIn('id', $seller_keys)
            ->get()
        ...
        foreach ($users as $user) {
            if (isset($sellers[$user['id']])) {
                $format_sellers[] = [
                    'id' => $user['id'],
                    'name' => $user['name'],
                    'payment' => $sellers[$user['id']]['payment'],
                    'gain' => $sellers[$user['id']]['gain'],
                    'comission' => $sellers[$user['id']]['comission'],
                ];
            } else {
                $format_sellers[] = [
                    'id' => $user['id'],
        ...
        $cash_egress = $egress_amount;
        $cash_beginning_balance = $cash->cash_beginning_balance;
        $document_credit_total = 0;
        $total_cash_efectivo = $cash_amount;
        $nota_debito = 0;
        $nota_credito = 0;
        $cash_user_name = $cash->user->name;
        $cash_total_net = $cash_amount - $cash_egress + $cash_beginning_balance;
        $total_tips = 0;
        $total_virtual = $methods_payments->where('is_digital', true)->sum('sum');
        $cpe_total_cash = $cash_cpe;
        $total_bank = $methods_payments->where('is_bank', true)->sum('sum');
        $sale_notes_total_cash = $cash_sale_note;
        $final_balance = $cash->final_balance_with_banks;
        $document_credit_total = 0;
        $final_balance_net = $final_balance + $cash_beginning_balance - $cash_egress;
        $methods_payment = $methods_payments->where('sum', '>', 0)->map(function ($item) {
            return  (array) $item;
       });
        $to_return = [
            'cash' => (object) [
            ...
            'cash_beginning_balance' => $cash_beginning_balance ?? 0.00,
            'document_credit_total' => $document_credit_total ?? 0.00,
            'total_cash_efectivo' => $total_cash_efectivo ?? 0.00,
            'nota_debito' => $nota_debito ?? 0.00,
            'nota_credito' => $nota_credito ?? 0.00,
            'cash_user_name' => $cash_user_name ?? '',
            'cash_total_net' => $cash_total_net ?? 0.00,
            'total_tips' => $total_tips ?? 0.00,
            'total_virtual' => $total_virtual ?? 0.00,
            'cpe_total_cash' => $cpe_total_cash ?? 0.00,
            'total_bank' => $total_bank ?? 0.00,
            'sale_notes_total_cash' => $sale_notes_total_cash ?? 0.00,
            'cash_income' => $methods_payment->sum('sum') ?? 0.00,
            'final_balance' => $final_balance ?? 0.00,
            'document_credit_total' => $document_credit_total ?? 0.00,
            'final_balance_net' => $final_balance_net ?? 0.00,
            'success' => true,
            'cash_cpe' => $cash_cpe ?? 0.00,
            'cash_sale_note' => $cash_sale_note ?? 0.00,
            'cash_egress' => $cash_egress ?? 0.00,
            'sellers' => $format_sellers,
            'cpe_amount' => $cpe_amount ?? 0.00,
            'sale_note_amount' => $sale_note_amount ?? 0.00,
            'egress_amount' => $egress_amount ?? 0.00,
            'methods_payment' => $methods_payment->toArray(),
            'cash_amount' => $cash_amount ?? 0.00,
            'cpe_total' => $cpe_amount,
            'sale_notes_total' => $sale_note_amount,
        ];
        return $to_return;
    }

    public function getGainByItems($items)
    {
        $gain = 0;
        $comission = 0;
        $item_ids_to_query = [];
        ...
        $bank_documents = DocumentPayment::with([
            ...
            ->where(function ($query) use ($cash_id) {
                ...
                return (object)[
                    'payment_id' => $item->id,
                    'payment_type' => 'App\Models\Tenant\DocumentPayment',
                    'destination_type' => 'App\Models\Tenant\BankAccount',
                ];
            });
