...
$company_name = '';
if ($multi_companies && $document->website_id) {
...
$idAttributeSelect = isset($value->item->idAttributeSelect) ? $value->item->idAttributeSelect : [];
...
<tr>
    ...
    @if ($multi_companies)
        <td class="celda">{{ $company_name }}</td>
    @endif
    ...
    <td class="celda">
        @if ($isSaleNote)
            ...
            @if ($isQuotation)
                COTIZACIÓN
            @else
                @if (isset($document->document_type) && !isset($document->document_type->description))
                    NOTA DE VENTA
                @endif

                {{ isset($document->document_type->description) ? $document->document_type->description : 'NOTA DE VENTA' }}
            @endif
        @else
            NOTA DE VENTA
        @endif
    ...
    @if ($configuration->show_item_attributes)
        @php
            $brand = '';
            $year = '';
            $color = '';
            ...
            if (isset($idAttributeSelect)) {
                if (is_array($idAttributeSelect)) {
                    $brand = $idAttributeSelect[0]->attribute ?? '';
                    $year = $idAttributeSelect[0]->attribute5 ?? '';
                    $color = $idAttributeSelect[0]->attribute3 ?? '';
                    $motor = $idAttributeSelect[0]->attribute4 ?? '';
                    $serie = $idAttributeSelect[0]->chassis ?? '';
                    $model_property = $idAttributeSelect[0]->attribute2 ?? '';
                } elseif (is_object($idAttributeSelect)) {
                    $brand = $idAttributeSelect->attribute ?? '';
                    $year = $idAttributeSelect->attribute5 ?? '';
                    $color = $idAttributeSelect->attribute3 ?? '';
                    $motor = $idAttributeSelect->attribute4 ?? '';
                    $serie = $idAttributeSelect->chassis ?? '';
                    $model_property = $idAttributeSelect->attribute2 ?? '';
                }
            }
        @endphp
        <th>{{ $brand }}</th>
    ...
    @endif
    <td class="celda">
        @if($configuration->show_item_attributes && $model_property !== '')
            {{ $model_property }}
        @else
            {{ $model }}
        @endif
