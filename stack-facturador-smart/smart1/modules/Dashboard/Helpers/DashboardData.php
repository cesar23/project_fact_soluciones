...
class DashboardData
{
    ...
    public function globalData($request = null)
    {
        $from_api = request()->is('*/api/*');
        $start_date = null;
        $end_date = null;
        $establishment_id = null;
        if($request){
            $period = $this->getPeriod($request);
            $start_date = $period['d_start'];
            $end_date = $period['d_end'];
            $establishment_id = isset($request['establishment_id']) ? $request['establishment_id'] : null;
        }
        $document_global = $this->document_totals_globals( $start_date, $end_date, $establishment_id );
        $document_total_global = $document_global['total'];
        $quantity_documents = $document_global['quantity_documents'];
        return [
            'total_cpe_all' => Configuration::first()->quantity_documents,
            'total_cpe' => $quantity_documents,
            'document_total_global' => $document_total_global,
            'quantity_documents' => $quantity_documents,
            'sale_note_total_global' => $this->sale_note_totals_global( $start_date, $end_date, $establishment_id ),
        ];
    }
    ...
    public function document_totals_globals($start_date = null, $end_date = null,$establishment_id = null)
    {
        $documents_query = Document::without(['user', 'soap_type', 'state_type', 'document_type', 'currency_type', 'group', 'items', 'invoice', 'note'])
            ->select('id', 'state_type_id', 'document_type_id','number', 'currency_type_id', 'total', 'exchange_rate_sale');
        ...
        $quantity_documents = $documents->count();
        //PEN
        $document_total_pen = 0;
        $document_total_payment_pen = 0;
        $document_total_note_credit_pen = 0;
