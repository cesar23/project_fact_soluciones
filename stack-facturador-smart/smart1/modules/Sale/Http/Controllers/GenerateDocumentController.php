...
use Modules\Sale\Http\Resources\TechnicalServiceResource;
    ...
    class GenerateDocumentController extends Controller
    {
        ...
        public function tables(Request $request)
        {
            ...
            if ($type === 'technical-services') {
                $document_types[] = ['id' => 'cot', 'name' => 'Cotización'];
            }
        ...
        public function store(Request $request)
        {
            ...
            try {
                ...
                foreach ($items as $index => $item) {
                    ...
                    $inputs['items'][$index]['warehouse_id'] = $inputs['establishment_id'];
                }
                ...
                if (in_array($request->input('document_type_id'), ['01', '03'])) {
                    ...
                    if ($request->input('document_type_id') === 'nv') {
                        $res = (new SaleNoteController())->storeWithData($inputs);
                    } else {
                        $quotation_request = new QuotationRequest();
                        $quotation_request->replace($inputs);
                        $quotation_request->merge(['items' => $inputs['items']]);
                        $quotation_request->merge(['prefix' => $inputs['series']]);
                        $res = (new QuotationController())->store($quotation_request);
                    }
                }
