                ...
                <table class="">
                    ...
                    <tbody>
                        @foreach ($records as $key => $value)
                            ...
                            @if ($total_to_pay > 0)
                                <tr>
                                    <td class="celda {{ $dued }}">{{ $value['supplier_name'] }}</td>
                                    <td class="celda text-center {{ $dued }}">
                                        {{ $value['number_full'] }}</td>
                                    <td class="celda text-center {{ $dued }}">{{ $value['date_of_issue'] }}
                                    </td>
                                    <td class="celda text-center {{ $dued }}">{{ strtoupper($value['type']) }}</td>
                                    <td class="celda text-center {{ $dued }}">
                                        {{ $value['currency_type_id'] === 'PEN' ? 'S/' : '$' }}
                                    </td>
                                    <td class="celda text-end {{ $dued }}">{{ number_format($total_value, 2) }}</td>
                                    <td class="celda text-end {{ $dued }}">{{ number_format($total_payment, 2) }}</td>
                                    <td class="celda text-end {{ $dued }}">{{ number_format($total_to_pay, 2) }}</td>
                                    <td class="celda {{ $dued }}">{{ $value['supplier_telephone'] ?? '-' }}</td>
                                    <td class="celda {{ $dued }}">{{ $dued ? 'VENCIDO' : 'PENDIENTE' }}</td>
                                    <td class="celda text-end {{ $dued }}">{{ $value['delay_payment'] ?? 0 }}</td>
                    ...
                    <tfoot>
                        <tr>
                            <td colspan="2"></td>
                            <td class="celda text-end" colspan="3">TOTAL S/</td>
                        ...
                        <tr>
                            <td colspan="2"></td>
                            <td class="celda text-end" colspan="3">TOTAL $</td>
