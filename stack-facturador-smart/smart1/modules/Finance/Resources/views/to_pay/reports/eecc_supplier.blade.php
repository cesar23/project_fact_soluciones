...
<body>
    ...
    @php
        ...
        foreach ($records as $value) {
            ...
            // Convertimos la fecha de string a Carbon si existe
            if (isset($value['date_of_due']) && $value['date_of_due']) {
                try {
                    // Intentamos diferentes formatos de fecha
                    $due_date = null;
                    if (strpos($value['date_of_due'], '/') !== false) {
                        // Formato d/m/Y
                        $due_date = \Carbon\Carbon::createFromFormat('d/m/Y', $value['date_of_due']);
                    } else {
                        // Formato Y-m-d o Y/m/d
                        $due_date = \Carbon\Carbon::parse($value['date_of_due']);
                    }
            ...
            <table class="no-border">
                <tr>
                    ...
                    <td width="33%" style="text-align: center;">
                        <h1>Estado de Cuentas por Pagar al {{ $date }}</h1>
                    </td>
                    <td width="33%">
                    </td>
                </tr>
                ...
                <tr>
                    <td class="credit-box">
                        <table class="credit-item-table">
                            <thead>
                                <tr>
                                    <th colspan="2" class="credit-box-title f14">Resumen de cuentas por pagar</th>

                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td class="credit-item-label f14">Total facturado:</td>
                                    <td class="credit-item-value f14">
                                        {{ number_format($total, 2, '.', ',') }} </td>
                                </tr>
                                <tr>
                                    <td class="credit-item-label f14">Total pagado:</td>
                                    <td class="credit-item-value f14">{{ number_format($total_payed, 2, '.', ',') }} </td>
                                </tr>
                                <tr>
                                    <td class="credit-item-label f14">Saldo pendiente:</td>
                                    <td class="credit-item-value f14">
                                        {{ number_format($total_debt, 2, '.', ',') }} </td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
                    <td class="credit-box">
                        <table class="credit-item-table">
                            <thead>
                                <tr>
                                    <th colspan="2" class="credit-box-title f14">Estado de pagos</th>
                        ...
                        <tbody>
                            @foreach ($records as $key => $value)
                                    @php
                                        ...
                                        $isDued = (isset($value['delay_payment']) && $value['delay_payment'] > 0) ? true : false;
                                    @endphp
                                    <tr>
                                        ...
                                        <td class="celda {{ $isDued ? 'text-danger' : '' }}">{{ $value['id'] }}</td>
                                        ...
                                        <td class="celda {{ $isDued ? 'text-danger' : '' }}">{{ $isDued ? 'Vencido' : 'Pendiente' }}</td>
                                        <td class="celda {{ $isDued ? 'text-danger' : '' }}">{{ $value['delay_payment'] ?? 0 }}</td>
                ...
                <tr>
                    <td>
                        <table class="payment-note-table">
                            ...
                            <tr>
                                <td colspan="2">(*) Estado de cuentas por pagar con proveedores</td>
                            </tr>
                            <tr>
                                <td colspan="2">(*) Para cualquier consulta contactar con el área de contabilidad.</td>
                            </tr>
                        </table>
                    </td>
                </tr>
