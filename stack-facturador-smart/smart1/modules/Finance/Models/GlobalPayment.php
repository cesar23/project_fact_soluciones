...
class GlobalPayment extends ModelTenant
{
    ...
    {
        parent::boot();
        static::created(function (GlobalPayment $payment) {
            $destination = $payment->destination_type;
            if ($destination == Advance::class) {
                $payment_id = $payment->payment_id;
                $payment_type = $payment->payment_type;
                $payment_inst = app($payment_type)->find($payment_id);
                if ($payment_inst) {
                    $payment_amount = $payment_inst->payment;
                    $destination_id = $payment->destination_id;
                    $advance = Advance::find($destination_id);
                    $advance->discount($payment_amount);
                }
    ...
    public function bill_of_exchange_payment()
    {
    ...
    public function getInstanceTypeAttribute()
    {
        $instance_type = [
            ...
            BillOfExchangePayment::class => 'bill_of_exchange_payment',
        ];
    ...
    public function scopeWhereFilterPaymentType(Builder $query, $params)
    {
        ...
        $query
            ->OrWhereHas('bill_of_exchange_payment', function ($q) use ($params) {
                if ($params->date_start) {
                    $q->where('date_of_payment', '>=', $params->date_start);
                }
                if ($params->date_end) {
