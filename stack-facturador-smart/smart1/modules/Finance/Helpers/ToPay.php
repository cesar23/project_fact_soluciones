...
class ToPay
{
    ...
    public static function getToPay($request)
    {
        ...
        $period = $request['period'] ?? 'between_dates';
        ...
        $select = DB::raw('purchases.id as id, ' .
            ...
            "CONCAT(purchases.series,'-',
            purchases.number) AS number_full, " .
        ...
        $purchase_fees_select = "purchase_fee.id as id, " .
        "DATE_FORMAT(purchase_fee.date, '%Y/%m/%d') as date_of_issue, " .
        "DATE_FORMAT(purchase_fee.date, '%Y/%m/%d') as date_of_due, " .
        "persons.name as supplier_name," .
        "persons.id as supplier_id," .
        "purchases.document_type_id," .
        "CONCAT(purchases.series,'-',purchases.number,'-',purchase_fee.id) AS number_full, " .
        "IFNULL(purchase_fee.original_amount, purchase_fee.amount) as total, " .
        "CASE WHEN purchase_fee.is_canceled = 1 THEN purchase_fee.original_amount ELSE (purchase_fee.original_amount - purchase_fee.amount) END as total_payment, " .
        "CASE WHEN purchase_fee.is_canceled = 1 THEN 0 ELSE purchase_fee.amount END as total_subtraction, " .
        "'purchase_fee' AS 'type', " .
        "purchases.currency_type_id, " .
        "purchases.exchange_rate_sale, " .
        "purchases.user_id, " .
        "users.name as username";
        $purchases_fees = DB::connection('tenant')
        ->table('purchases')
        ->join('persons', 'persons.id', '=', 'purchases.supplier_id')
        ...
        ->select(DB::raw($purchase_fees_select));
        ...
        if ($d_start && $d_end) {
            ...
            $bills_of_exchange
                ->whereBetween('bills_of_exchange_pay.date_of_due', [$d_start, $d_end]);
            $purchases_fees
                ->whereBetween('purchase_fee.date', [$d_start, $d_end]);
        }
        if ($supplier_id !== 0) {
            $purchases->where('supplier_id', $supplier_id);
            $bills_of_exchange->where('supplier_id', $supplier_id);
            $purchases_fees->where('supplier_id', $supplier_id);
        }
        if($establishment_id !== 0) {
            $purchases->where('purchases.establishment_id', $establishment_id);
            $bills_of_exchange->where('bills_of_exchange_pay.establishment_id', $establishment_id);
            $purchases_fees->where('purchases.establishment_id', $establishment_id);
        }
        ...
        if ($withBankLoan === 1) {
            ...
            $select = 'bank_loans.id as id, ' .
                ...
                'IFNULL(payments.total_payment, 0) as total_payment, ' .
                '0 as total_subtraction, ';
            if ($d_start && $d_end) {
                $bankLoans
                    // ->where('supplier_id', $supplier_id)
                    ->select(DB::raw($select .
                        "'bank_loans' AS 'type', " . 
                        "bank_loans.currency_type_id, " . 
                        "bank_loans.exchange_rate_sale, " .
                        "null as user_id, " .
                        "null as username"))
        ...
        if ($user !== 0) {
            ...
            $bills_of_exchange->where('user_id', $user);
            $purchases_fees->where('user_id', $user);
        } else {
        ...
        $purchases_fees = $purchases_fees->havingRaw('CAST(total_subtraction as DECIMAL(12,2)) > 0');
        $records = $purchases->union($expenses);    
        $records->union($bills_of_exchange);
        $records->union($purchases_fees);
        if ($bankLoans !== null) {
