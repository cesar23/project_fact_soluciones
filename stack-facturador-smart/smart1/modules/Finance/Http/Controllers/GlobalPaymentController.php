...
use Modules\Finance\Exports\GlobalPaymentExportManual;
...
class GlobalPaymentController extends Controller
{
    ...
    public function filter()
    {
        ...
        $users = User::where('type', '!=', 'superadmin')->get()->transform(function($item){
            return [
                'id' => $item->id,
                'name' => $item->name,
            ];
        ...
        return compact('payment_types', 'destination_types', 'users');
    }
    ...
    public function excelManual(Request $request){
            $company = Company::first();
        $establishment = ($request->establishment_id) ? Establishment::findOrFail($request->establishment_id) : auth()->user()->establishment;
        $records = $this->getRecords($request->all(), GlobalPayment::class)->get();

        return (new GlobalPaymentExportManual)
                ->records($records)
                ->company($company)
