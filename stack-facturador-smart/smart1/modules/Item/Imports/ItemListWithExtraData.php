    ...
    class ItemListWithExtraData implements ToCollection
    {
        ...
        public function collection(Collection $rows)
        {
            ...
            foreach ($rows as $row) {
                ...
                $catIngredientAttributes = $row[11] ?? null;  //Atributos de ingredientes
                $catLineAttributes = $row[12] ?? null;  //Atributos de líneas
                ...
                if ($item !== null && !empty($internal_id)) {
                    ...
                    $catIngredientAttributes = $this->findByNameOnTables(IngredientAttributeItem::query(), $catIngredientAttributes)->get();
                    $catLineAttributes = $this->findByNameOnTables(LineAttributeItem::query(), $catLineAttributes)->get();
                     ...
                     * @var \Illuminate\Database\Eloquent\Collection $catIngredientAttributes
                     * @var \Illuminate\Database\Eloquent\Collection $catLineAttributes
                     */
                    ...
                    // Manejar atributos de ingredientes y líneas
                    if ($catIngredientAttributes->count() > 0 || $catLineAttributes->count() > 0) {
                        $this->setItemAttributes($item, $catIngredientAttributes, $catLineAttributes);
                    }
        ...
        /**
         * Establece los atributos de ingredientes y líneas para un item
         *
         * @param Item $item
         * @param \Illuminate\Database\Eloquent\Collection $ingredientAttributes
         * @param \Illuminate\Database\Eloquent\Collection $lineAttributes
         */
        protected function setItemAttributes($item, $ingredientAttributes, $lineAttributes)
        {
            // Obtener todas las combinaciones posibles
            $combinations = [];

            if ($ingredientAttributes->count() > 0 && $lineAttributes->count() > 0) {
                // Si hay ambos, crear combinaciones de ingrediente + línea
                foreach ($ingredientAttributes as $ingredient) {
                    foreach ($lineAttributes as $line) {
                        $combinations[] = [
                            'cat_ingredient_id' => $ingredient->id,
                            'cat_line_id' => $line->id
                        ];
                    }
                }
            } elseif ($ingredientAttributes->count() > 0) {
                // Solo ingredientes
                foreach ($ingredientAttributes as $ingredient) {
                    $combinations[] = [
                        'cat_ingredient_id' => $ingredient->id,
            ...
            } elseif ($lineAttributes->count() > 0) {
                // Solo líneas
                foreach ($lineAttributes as $line) {
                    $combinations[] = [
                        'cat_ingredient_id' => null,
                        'cat_line_id' => $line->id
                    ];
                }
            }

            // Eliminar atributos existentes para este item
            ...
            foreach ($combinations as $combination) {
                ItemAttribute::create([
                    'item_id' => $item->id,
                    'cat_ingredient_id' => $combination['cat_ingredient_id'],
                    'cat_line_id' => $combination['cat_line_id']
                ]);
            }
        }
