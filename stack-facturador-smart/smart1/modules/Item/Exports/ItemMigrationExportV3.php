...
class ItemMigrationExportV3 implements ShouldAutoSize, FromCollection, WithHeadings, WithMapping, WithStyles, WithEvents
{
    use Exportable;
    ...
    public function headings(): array
    {
        ...
        foreach ($this->warehouses as $warehouse) {
            $warehouseHeadings[] = 'Stock ' . $warehouse->description;
        }
    ...
    public function map($item): array
    {
        $baseData = [
            ...
            $item->sale_unit_price ?? '0.00',
            $item->sale_affectation_igv_type_id ?? '',
            $item->has_igv ? 'SI' : 'NO',
            $item->purchase_unit_price ?? '0.00',
        ...
        foreach ($this->warehouses as $warehouse) {
            $stock = 0;
            $itemWarehouse = DB::connection('tenant')->table('item_warehouse')->where('item_id', $item->id)->where('warehouse_id', $warehouse->id)->first();
        ...
        $remainingData = [
            ...
            $item->name ?? '',
            $item->secondary_name ?? $item->second_name ?? '',
            $item->lot_code ?? '',
            $item->date_of_due ? Carbon::parse($item->date_of_due)->format('d/m/Y') : '',
            $item->barcode ?? '',
            $item->info_link ?? ''
        ];
