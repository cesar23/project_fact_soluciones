...
trait CashReportTrait
{
    ...
    public function getDataPaymentsAssociatedCash($cash, &$data)
    {
        ...
        foreach ($documents_payments as $document_payment) {
            ...
            $total_income += $document_payment->payment;
            $payments->push($document_payment->getRowResourceCashPayment());
        }
        ...
        foreach ($sale_note_payments as $payment) {
            $total_income += $payment->payment;
            $payments->push($payment->getRowResourceCashPayment());
        }
    ...
    public function setDataCashSalesPurchases($cash, &$data)
    {
        ...
        foreach ($documents_payments as $document_payment) {
            ...
            if ($document_payment->payment_method_type->is_cash) {
                $data['cash_sales_income']['total_cash'] += $document_payment->payment;
            } else if ($document_payment->payment_method_type->is_bank) {
                $data['cash_sales_income']['total_transfer'] += $document_payment->payment;
            }
        }
        ...
        foreach ($sale_note_payments as $payment) {
            ...
            if ($payment->payment_method_type->is_cash) {
                $data['cash_sales_income']['total_cash'] += $payment->payment;
            } else if ($payment->payment_method_type->is_bank) {
                $data['cash_sales_income']['total_transfer'] += $payment->payment;
            }
        }
        ...
        foreach ($purchase_payments as $payment) {
            ...
            if ($payment->payment_method_type->is_cash) {
                $data['purchase_cash']['total_cash'] += $payment->payment;
            } else if ($payment->payment_method_type->is_bank) {
                $data['purchase_cash']['total_transfer'] += $payment->payment;
            }
        }

        $data['purchase_cash']['total'] = $data['purchase_cash']['total_cash'] + $data['purchase_cash']['total_transfer'];

        $data['cash_sales_income']['total'] = $data['cash_sales_income']['total_cash'] + $data['cash_sales_income']['total_transfer'];
    }
