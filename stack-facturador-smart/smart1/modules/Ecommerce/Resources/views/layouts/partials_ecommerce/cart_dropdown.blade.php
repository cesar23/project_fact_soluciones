...
<script type="text/javascript">
	...
	function calculatetotal()
	{
		...
		array.forEach(element => {
			console.log(element);
			let sale_unit_price = element.sale_unit_price;
			if(element.item_unit_types){
				price_store = element.item_unit_types.find(x => x.default_price_store == 1);
				if(price_store){
			...
			total += parseFloat(sale_unit_price)
		});
				...
				$(".dropdown-cart-products").append( `
						<div class="product">
							...
							<h4 class="product-title">
								<a href="${element.id}">${element.description}</a>
							</h4>
