...
class DocumentController extends Controller
{
    ...
    public function data_table()
    {
        ...
        return compact('customers', 'document_types', 'series', 'establishments', 'state_types', 'companies');
    }
    ...
    public function prepayments($type, $customer_id = null)
    {

        $prepayment_documents = Document::whereHasPrepayment()->whereAffectationTypePrepayment($type);
        if($customer_id){
            $prepayment_documents = $prepayment_documents->where('customer_id', $customer_id);
        }
        
        
        $prepayment_documents = $prepayment_documents->get()->transform(function ($row) {
    ...
    public function searchItems(Request $request)
    {
        ...
        $warehouses = DB::connection('tenant')->table('warehouses')->select('id','description','establishment_id')->where('establishment_id', auth()->user()->establishment_id)->get();
        Cache::put("warehouses_".$user_id, $warehouses, now()->addMinutes(5));
    ...
    public function searchItemsFavorite(Request $request)
    {

        $user_id = auth() ? auth()->id() : 1;
        $items = Item::where('frequent', true)->where(function($query) use ($request){
    ...
    public function searchItemsLite(Request $request)
    {

        $user_id = auth() ? auth()->id() : 1;
        Cache::put("request_type_".$user_id, "document", 30);
        $items = SearchItemController::getItemsToDocumentsLite($request);
