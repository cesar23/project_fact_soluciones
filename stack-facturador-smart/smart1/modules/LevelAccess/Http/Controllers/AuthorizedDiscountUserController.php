...
class AuthorizedDiscountUserController extends Controller
{
    ...
    public function checkToken(Request $request)
    {
        ...
        if ($authorized_discount_user) {
            return $this->generalResponse(true, 'Token válido');
        }
    ...
    public function validateTokens(Request $request)
    {
        ...
        AuthorizedDiscountUser::whereIn('token', $tokens)->update([
            'seller_id' => auth()->id(),
            'active' => false,
        ]);

        return $this->generalResponse(true, 'Tokens validados correctamente.');
    }
