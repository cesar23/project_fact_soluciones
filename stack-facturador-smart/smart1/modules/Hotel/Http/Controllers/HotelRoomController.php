...
class HotelRoomController extends Controller
{
	...
	public function searchRooms(Request $request){
		...
		if($input){
			$rooms->where('name', 'like', '%' . $input . '%')
				->orWhere('description', 'like', '%' . $input . '%');
		}
		$rooms = $rooms->limit(10)->get();
	...
	{
		try {
			$reservation = HotelReservation::where('room_id', $id)
				->where('active', true)
			->first();
			if($reservation){
				return response()->json([
					'success' => false,
					'data'    => 'La habitación no se puede eliminar porque tiene una reserva asociada'
				], 500);
			}
			HotelReservation::where('room_id', $id)->delete();
	...
	public function verifyRoomStatus($roomId)
	{
