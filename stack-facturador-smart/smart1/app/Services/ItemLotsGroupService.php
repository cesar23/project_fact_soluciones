...
class ItemLotsGroupService
{
     ...
     */
    public function getLote($id, $separator = '|')
    {
        $result = '';
        if (is_array($id)) {
            foreach ($id as $item) {
                if (isset($item->compromise_quantity) && $item->compromise_quantity > 0) {
                    $result .= $separator . $item->code;
                }
                // $result .= "/" . $item->code;
            }
        } else {
     ...
     * @return \Illuminate\Database\Eloquent\HigherOrderBuilderProxy|mixed|string
     */
    public function getLotDateOfDue($id,$separator = '|')
    {
        $result = '';
        if (is_array($id)) {
            foreach ($id as $item) {
                if (isset($item->compromise_quantity) && $item->compromise_quantity > 0) {
                    $result .= $separator  . $item->date_of_due;
                }
        ...
        if (substr($result, 0, 1) == $separator) {
            $result = substr($result, 1);
        }
        return $result;
    }
