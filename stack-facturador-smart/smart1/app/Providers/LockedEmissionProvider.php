...
class LockedEmissionProvider extends ServiceProvider
{
    ...
    private function locked_users()
    {
        ...
        User::creating(function ($user) {
            ...
            $quantity_users = User::where('type', '!=', 'superadmin')->count();
            $plan = $this->getClientPlan(['id', 'name', 'limit_users']);
            if($configuration->locked_users &&  $plan->limit_users !== 0){

                if($quantity_users >= $plan->limit_users )
                {
