...
class SearchCustomerController extends Controller
{
    ...
    public static function getCustomersToSuscriptionList(Request $request = null, ?int $id = 0, $onlyParent = false)
    {
        ...
        $orderColum = 'name';
        $children = false;
        $full_suscription_credit = $request->full_suscription_credit;
        $pending = $request->pending;
        if($full_suscription_credit){
            $person->whereHas('full_suscription_credit', function($query){
        ...
        if($pending){
            $person->whereExists(function($query){
                $query->select(DB::raw(1))
                ->from('sale_notes')
                ->whereColumn('sale_notes.customer_id', 'persons.id')
                ->where('sale_notes.user_rel_suscription_plan_id', '>', 0)
                ->where('sale_notes.total_canceled', false)
                ->whereRaw('sale_notes.total > (
                    SELECT COALESCE(SUM(sale_note_payments.payment), 0) 
                    FROM sale_note_payments 
                    WHERE sale_note_payments.sale_note_id = sale_notes.id
                )');
            });
        }
        ...
        if ($request != null) {
            ...
            if ($request->has('section') && !empty($request->section)) {
                ...
                $person->whereHas('student', function ($q) use ($section) {
                    $q->where('section', $section);
                });
            }
            if ($request->has('grade') && !empty($request->grade)) {
                $grade = $request->grade;
                $person->whereHas('student', function ($q) use ($grade) {
                    $q->where('grade', $grade);
                });
            }
            if ($request->has('parent_id') && !empty($request->parent_id)) {
                $parent_id = $request->parent_id;
                $person->where('parent_id', $parent_id);
            }
            if ($request->has('column') && !empty($request->column)) {
        ...
        if ($request->has('users') && !empty($request->users) && $request->users == 'children') {
        }
        return $person
            ->whereType('customers')
            ->whereIsEnabled()
