...
class VoidedController extends Controller
{
    ...
    public function getRecords(Request $request)
    {
        $voided = DB::connection('tenant')
            ...
            ->select(DB::raw("id,ticket,'voided' AS 'type'"));
    ...
    public function store(VoidedRequest $request)
    {
        ...
        if($configuration->audit_sales){
            $documents = $document->documents;
            foreach($documents as $doc){
                $doc->document->auditVoided('Anulado');
            }
        }
        $items = DocumentItem::where('document_id', $document->id)->get();
    ...
    public function status($statusTicket)
    {
        $document = Voided::where("id", $statusTicket)->first();
        if (!$document) {
            $company = Company::active();
            Log::error(json_encode([
                'success' => false,
                'message' => 'No se encontró la anulación',
                'ticket' => $statusTicket,
