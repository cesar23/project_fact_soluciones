...
class SummaryController extends Controller
{
    ...
    public function recordsToSend() {
        $records = Summary::where('summary_status_type_id', '3')
    ...
    public function store(SummaryRequest $request) {
        ...
        // Revertir pagos de deuda si corresponde
        if (isset($request->documents) && is_array($request->documents)) {
            foreach ($request->documents as $documentData) {
                if (isset($documentData['document_id'])) {
                    \App\Services\DebtReversalService::reverseDebtPayments(
