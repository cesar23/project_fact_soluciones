...
class CashController extends Controller
{
    ...
    public function columns()
    {
        ...
        $cash_by_establishment = $configuration->cash_by_establishment;
        if ($cash_by_establishment) {
            return [
    ...
    public function records(Request $request)
    {
        ...
        if ($currency_type_id) {
            $records = $records->where('currency_type_id', $currency_type_id);
        }
        if ($user->type === 'admin_lower') {
    ...
    public function getExchangeRateSale(Request $request){
        $date = $request->date;
        $exchange_rate_sale = ExchangeCurrency::where('date', $date)->first();
        if(!$exchange_rate_sale){
            return [
                'success' => false, 
                'message' => 'No se encontró la conversión de cambio para la fecha seleccionada'
            ];
        }
        return [
            'success' => true,
    ...
    public function tables()
    {
        ...
        $establishments = Establishment::where('active', true)->get();
        switch ($type) {
            case 'admin':
        ...
        return compact('users', 'user', 'companies', 'establishments');
    }
     ...
     * @return array
     */
    public function opening_cash_current_user()
    {
        if (auth('api')->check()) {
            $user = auth('api')->user();
            if (!$user) {
    ...
    public function opening_cash_check($user_id)
    {
        ...
        if ($cash_by_establishment) {
            ...
            $cash = Cash::where([['user_id', $user_id], ['state', true], ['establishment_id', $establishment_id]])->first();
        } else {
    ...
    public function store(CashRequest $request)
    {
        ...
        if ($cash_by_establishment && $id == null) {
            ...
            if ($cash) {
                return [
                    'success' => false,
                    'message' => 'Ya existe una caja aperturada para este establecimiento'
                ];
            }
        }
        ...
        if ($cash && $id == null) {
            return [
                'success' => false,
                'message' => 'Ya existe una caja aperturada para este usuario'
            ];
        }
        ...
        DB::connection('tenant')->transaction(function () use ($id, $request) {
            ...
            if ($website_id) {
                ...
                if ($company_found) {
                    $cash->company = $company_found->name;
                    $cash->website_id = $company_found->website_id;
                }
            }
    ...
    public function getInitialBalance(Request $request){
        ...
        $cash_by_establishment = $configuration->cash_by_establishment;
        if ($cash_by_establishment) {
            $cash = Cash::where([['establishment_id', $establishment_id]])->where('final_balance_to_next_cash', true)->latest()->first();
            if($cash){
                return [
                    'success' => true,
                    'initial_balance' => $cash->final_balance_with_banks
                ];
            }
        }else{
            $cash = Cash::where([['user_id', $user_id]])->where('final_balance_to_next_cash', true)->latest()->first();
            if($cash){
                return [
                    'success' => true,
                    'initial_balance' => $cash->final_balance_with_banks
    ...
    public function close($id, Request $request)
    {
        ...
        $cash = Cash::findOrFail($id);
        $cash->final_balance_to_next_cash = $request->final_balance_to_next_cash ?? false;
        ...
        foreach ($cash_documents as $cash_document) {
            ...
            // cotizacion
                ...
                if ($quotation_payment && $quotation_payment->quotation) {
                    $final_balance += ($quotation_payment->quotation->applyQuotationToCash()) ? $quotation_payment->quotation->getTransformTotal() : 0;
                }
        ...
        $document_payments = DocumentPayment::where(function ($query) use ($cash_id) {
            ...
            ->with(['document' => function ($query) use ($valid_state_types) {
                $query->select('id', 'currency_type_id', 'exchange_rate_sale', 'state_type_id')
                    ->whereIn('state_type_id', $valid_state_types);
            }])
            ->get();
        ...
        $sale_note_payments = SaleNotePayment::whereHas('sale_note', function ($query) use ($cash_id, $valid_state_types) {
            $query->where('cash_id', $cash_id)
                ->whereIn('state_type_id', $valid_state_types);
        })
            ->whereHas('global_payment', function ($query) {
        ...
        $cash->final_balance = round($final_balance + $cash->beginning_balance, 2);
        $cash->final_balance_with_banks = round($final_balance_with_banks +  $cash->final_balance, 2);
        $cash->income = round($final_balance, 2);
        $cash->state = false;
        $cash->save();
    ...
    public function report_products_ticket($id, $is_garage = false)
    {
        ...
        $pdf = Pdf::loadView('tenant.cash.report_product_ticket_pdf', $data);
        $filename = "Reporte_POS_PRODUCTOS - {$data['cash']->user->name} - {$data['cash']->date_opening} {$data['cash']->time_opening}";
        $height_by_document = 40;
        $height_total = $height_by_document * count($data['documents']);

        return $pdf->setPaper(array(0, 0, 226.77, $height_total + 250), 'portrait')->stream($filename . '.pdf');
    }
    ...
    public function getSaleNotesReportProducts($cash)
    {
        ...
        $cd_sale_notes = collect($cd_sale_notes)->merge($sale_notes)->unique();
        $sale_note_items = SaleNoteItem::with('sale_note')->whereIn('sale_note_id', $cd_sale_notes)
