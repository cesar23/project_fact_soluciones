...
class OrderCollection extends ResourceCollection
{
    ...
    public function toArray($request)
    {
        ...
        return $this->collection->transform(function($row, $key) {
            ...
            if($row->shipping_address && !empty($row->shipping_address) && $row->shipping_address != "direccion 1"){
                $customer_direccion = $row->shipping_address;
            }
            return [
                'id' => $row->id,
                ...
                'customer_direccion' => $customer_direccion,
                'items' => $row->items,
                'total' => $row->total,
