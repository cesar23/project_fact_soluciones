...
class DispatchRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            // 'customer_id'=> [
            //     'required_if:document_type_id, "09"',
            //     'required_unless:transfer_reason_type_id,"18"',
            // ],
            'customer_id' => [
                function ($attribute, $value, $fail) {
                    $documentTypeId = request()->input('document_type_id');
                    $transferReasonTypeId = request()->input('transfer_reason_type_id');

                    if ($documentTypeId === '09' && $transferReasonTypeId !== '18') {
                        if (empty($value)) {
                            $fail('El campo cliente es obligatorio cuando tipo de documento es 09.');
                        }
                    }
                }
            ],
