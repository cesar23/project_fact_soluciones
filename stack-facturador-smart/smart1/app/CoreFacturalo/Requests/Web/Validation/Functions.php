...
class Functions
{
    ...
    public static function findSeries($inputs)
    {
        ...
        if (key_exists('series', $inputs)) {
            ...
            $series = Series::query()
                ...
                $series =   $series->where('establishment_id', $inputs['establishment_id']);
                }
                $series = $series->first();
                return $series;
        } else {
            if (!$inputs['series_id']) {
                ...
                if (key_exists('items', $to_log)) {
                    unset($to_log['items']);
                }
                $to_log['company_debug'] = [
                    'name' => $company->name,
                ...
                Log::debug("La serie no existe", $to_log);
                throw new Exception("La serie no existe");
                } catch (\Exception $e) {
                    Log::error("Error en findSeries: " . $e->getMessage());
                    throw new Exception("La serie no existe");
                }
            };
    ...
    public static function DNI($inputs)
    {
        ...
        if (($inputs['document_type_id'] == '03') && ($inputs['total']) > 700 && $inputs['operation_type_id'] != "0401") {
            ...
            if (!in_array($person->identity_document_type_id, ['1', '6', '4', '7', '24'], true)) throw new Exception("El tipo doc. identidad {$person->identity_document_type->description} del cliente no es valido.");
        }
    }
