...
class Functions
{
    ...
    {
        $establishment = Establishment::where('code', $inputs['code'])
    ...
    {
        Log::info("Linea 146 - Entrada: " . json_encode($inputs));
        $exist_internal_id = isset($inputs['internal_id']) ? $inputs['internal_id'] : null;

        if ($exist_internal_id) {
            ...
            $inputs['internal_id'] = substr(md5($inputs['description']), 0, 10);
            $item = Item::firstOrCreate([
                'description' => $inputs['description'],
            ], [
                'internal_id' => $inputs['internal_id'],
    ...
    public static function voidedDocuments($inputs, $type)
    {
        ...
        foreach ($inputs['documents'] as $row) {
            ...
            if (!$document) throw new Exception("El código externo {$row['external_id']} no fue encontrado o la fecha indicada no corresponde al documento.");
