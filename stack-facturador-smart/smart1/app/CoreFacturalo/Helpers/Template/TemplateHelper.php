...
use App\Models\Tenant\Zone;
    ...
    /**
         ...
         * @return array
         */
        public static function getDetailedPaymentById( $id, $type_document, $dateFormat = 'Y-m-d')
        {
            $data = [];
            $document = null;
            if($type_document == 'document'){
                $document = Document::find($id);
            }elseif($type_document == 'sale_note'){
                $document = SaleNote::find($id);
            }elseif($type_document == 'quotation'){
                $document = Quotation::find($id);
            }
            $payments = $document->payments;
            if ($document->payment_condition_id === '01') {
                ...
                if(!empty($document->fee)) {
                    ...
                    foreach ($document->fee as $key => $quote) {
                        ...
                        if(is_string($date_quote)){
                            $date_quote = \Carbon\Carbon::parse($date_quote);
                        }
                        $temp = [
                            'description' => (empty($quote->getStringPaymentMethodType()) ? 'Cuota #' . ($key + 1) : $quote->getStringPaymentMethodType()),
                            'reference' =>  $date_quote->format($dateFormat),
                            'amount' => $quote->amount,
                            'symbol' => $quote->symbol,
                        ];
                        $data['CUOTA'][] = $temp;
