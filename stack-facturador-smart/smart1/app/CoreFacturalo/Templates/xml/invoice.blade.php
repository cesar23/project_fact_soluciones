...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    @foreach($document->related_document as $related)
    <cac:AdditionalDocumentReference>
        <cbc:ID>{{ $related->related_document }}</cbc:ID>
        <cbc:DocumentTypeCode>{{ $related->document_type_id }}</cbc:DocumentTypeCode>
    </cac:AdditionalDocumentReference>
    @endforeach
    @endif
    @if($document->related)
    ...
    <cac:AdditionalDocumentReference>
        ...
        <cac:IssuerParty>
            <cac:PartyIdentification>
                <cbc:ID schemeID="6">{{ trim($company->number) }}</cbc:ID>
            </cac:PartyIdentification>
        </cac:IssuerParty>
    </cac:AdditionalDocumentReference>
    @endforeach
    ...
    <cac:Signature>
        ...
        <cac:SignatoryParty>
            <cac:PartyIdentification>
                <cbc:ID>{{ trim($company->number) }}</cbc:ID>
            </cac:PartyIdentification>
            <cac:PartyName>
        ...
        <cac:Party>
            ...
            <cac:PartyLegalEntity>
                ...
                <cac:RegistrationAddress>
                    ...
                    @isset($establishment->urbanization)
    ...
    <cac:AccountingCustomerParty>
        <cac:Party>
            <cac:PartyIdentification>
                <cbc:ID schemeID="{{ $customer->identity_document_type_id }}">{{ trim($customer->number) }}</cbc:ID>
            </cac:PartyIdentification>
            <cac:PartyLegalEntity>
    ...
    <cac:AllowanceCharge>
        ...
        <cbc:Amount currencyID="{{ $document->currency_type_id }}">{{ number_format($charge->amount, 2, '.', '') }}</cbc:Amount>
    ...
    <cac:AllowanceCharge>
        ...
        <cbc:Amount currencyID="{{ $document->currency_type_id }}">{{ $document->generalApplyNumberFormat($discount->amount, 2) }}</cbc:Amount>
    ...
    <cac:InvoiceLine>
        ...
        @if(isset($document->detraction->trip_detail) && $document->detraction->trip_detail && isset($document->detraction->origin_location_id) && $document->detraction->origin_location_id && isset($document->detraction->origin_address) && $document->detraction->origin_address)
        <cac:Delivery>
            <cac:Despatch>
                ...
                <cac:DespatchAddress>
                    @if(isset($document->detraction->origin_location_id[2]) && $document->detraction->origin_location_id[2])
        ...
        @endif
        @if(isset($document->detraction->reference_value_effective_load) && $document->detraction->reference_value_effective_load)
        <cac:Delivery>
            <cac:DeliveryTerms>
                <cbc:ID>02</cbc:ID>
        ...
        @endif
        @if(isset($document->detraction->delivery_location_id) && $document->detraction->delivery_location_id && isset($document->detraction->delivery_address) && $document->detraction->delivery_address)
        <cac:Delivery>
            @if(isset($document->detraction->delivery_location_id) && $document->detraction->delivery_location_id)
            <cac:DeliveryLocation>
                <cac:Address>
                    @if(isset($document->detraction->delivery_location_id[2]) && $document->detraction->delivery_location_id[2])
        ...
        <cac:Item>
            ...
            <cbc:Description><![CDATA[{{ strlen($row->name_product_xml) > 250 ? substr($row->name_product_xml, 0, 250) : $row->name_product_xml }}]]></cbc:Description>
            @else
            <cbc:Description><![CDATA[{{ strlen($row->item->description) > 250 ? substr($row->item->description, 0, 250) : $row->item->description }}]]></cbc:Description>
            @endif
