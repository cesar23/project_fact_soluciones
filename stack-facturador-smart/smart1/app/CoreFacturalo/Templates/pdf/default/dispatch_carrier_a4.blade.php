...
<body>
    @if ($document->state_type->id == '11' || $document->state_type->id == '09')
        <div class="company_logo_box" style="position: absolute; text-align: center; top:30%;">
            <img src="data:{{ mime_content_type(public_path('status_images/anulado.png')) }};base64, {{ base64_encode(file_get_contents(public_path('status_images/anulado.png'))) }}"
                alt="anulado" class="" style="opacity: 0.6;">
        </div>
    @endif
    @if ($document->soap_type_id == '01')
        <div class="company_logo_box" style="position: absolute; text-align: center; top:30%;">
            <img src="data:{{ mime_content_type(public_path('status_images/demo.png')) }};base64, {{ base64_encode(file_get_contents(public_path('status_images/demo.png'))) }}"
                alt="demo" class="" style="opacity: 0.6;">
        </div>
    @endif
        ...
        <tbody>
            ...
            @if ($document->transport_data)
                ...
                @if (isset($document->transport_data['tuc']))
                    <tr>
                        <td width="50%" class="text-left desc"><strong>Tarjeta Única de Circulación
                                Principal:</strong>
                ...
                @if (isset($document->secondary_driver) && $document->secondary_driver->license)
                    <tr>
                        <td width="50%" colspan="2" class="text-left desc"><strong>Licencia del conductor
                                Secundario:</strong>
                ...
                <tr>
                    ...
                    @elseif (isset($document->channel))
                        <td width="50%" class="text-left desc"><strong>Canal:</strong>
                            {{ $document->channel->channel->channel_name }}
                        </td>
                    @endif
                </tr>
                @if(isset($document->transport_data['configuration']) && isset($document->channel))
                    <tr>
                        <td width="50%" class="text-left desc"><strong>Canal:</strong>
                            {{ $document->channel->channel->channel_name }}
                        </td>
        ...
        <tbody>
            @foreach ($document->items as $idx => $row)
                <tr>
                    ...
                    <td class="border-box bg-grey-light text-right desc">{{ number_format(isset($row->item->weight) ? $row->item->weight : 0, 2) }}
