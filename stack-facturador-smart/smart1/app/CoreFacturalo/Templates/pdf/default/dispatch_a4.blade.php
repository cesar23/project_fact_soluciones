@php
    ...
    $isIntegrateSystem = is_integrate_system();
    $company_name = $company->name;
    $show_weigth_dispatches = $configuration->show_weigth_dispatches;
    $company_owner = null;
...
<body>
    ...
    <table class="full-width mt-10">
        ...
        <tbody>
            ...
            @if ($isIntegrateSystem && $document->delivery)
                @php
                    $customer_id = $document->customer_id;
                    $address = \Modules\Dispatch\Models\DispatchAddress::where('person_id', $customer_id)
                        ->where('address', $document->delivery->address)
                        ->first();
                ...
                @if ($google_location)
                    <tr>
                        <td class="text-left desc" colspan="2">
                            <strong>Ubicación google:</strong>
                            <a href="{{ $google_location }}"
                                target="_blank">{{ strlen($google_location) > 100 ? substr($google_location, 0, 100) . '...' : $google_location }}</a>
                        </td>
                @endif
            @endif
            <tr>
            ...
            @else
                ...
                @if ($document->transport_data)
                    ...
                    @if (isset($document->transport_data['tuc']))
                        <tr>
                            <td width="50%" class="text-left desc "><strong>TUC:</strong>
                    ...
                    @if (isset($document->secondary_transport_data['tuc']))
                        <tr>
                            <td width="50%" class="text-left desc "><strong>TUC de placa secundaria:</strong>
                                {{ $document->secondary_transport_data['tuc'] ?? '' }}
                            </td>
                            <td></td>
                        </tr>
                    @endif
        ...
        <tbody>
            @foreach ($document->items as $idx => $row)
                <tr>
                    ...
                    @if ($show_weigth_dispatches)
                        <td class="border-box bg-grey-light text-right desc">
                            {{ number_format(isset($row->item->weight) ? $row->item->weight : 0, 2) }}
    ...
    @endif
    @if ($configuration->terms_condition_dispatches)
        <br>
        <table class="full-width">
            <tr>
                <td>
                    <h6 style="font-size: 12px; font-weight: bold;">Términos y condiciones del servicio</h6>
                    {!! $configuration->terms_condition_dispatches !!}
                </td>
            </tr>
        </table>
    @endif
