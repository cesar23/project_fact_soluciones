...
if ($logo) {
    ...
    $seller = $document->seller ? $document->seller->name : $document->user->name;
    if($document_affected){
        $seller = $document_affected->seller ? $document_affected->seller->name : $document_affected->user->name;
    }
    $affected_document_number = $document_affected
        ? $document_affected->series .
            '-' .
            str_pad($document_affected->number, 8, '0', STR_PAD_LEFT)
...
<table class="full-width">
    ...
    @endforeach
    <tr>
        <td class="desc"><strong>Vendedor:</strong>
            {{ $seller}}</td>
    </tr>
    ...
    <tr>
        <td class="text-center desc pt-5">Representación impresa del Comprobante de Pago Electrónico. Esta puede ser consultada en {!! searchUrl() !!}</td>
    </tr>
</table>
