...
<body>
    ...
    @endphp
    {{-- <div class="full-width border-box header-container" style="border-bottom: none;">
        <table
        class="full-width fixed-layout "
        >
        <thead class="">
            <tr class="bg-grey">
                <th class="desc text-center py-2" width="8%">CANT.</th>
                <th class="border-left desc text-center py-2" width="8%">UNIDAD</th>
                <th class="border-left desc text-left py-2" width="{{$width_description}}%" >DESCRIPCIÓN</th>
                @if (!$configurations->document_columns)

                    <th class="border-left desc text-center desc py-2" width="{{ $width_column}}%">P.Unit</th>
                    <th class="border-left desc text-center desc py-2" width="8%">Dto.</th>
                    <th class="border-left desc text-center desc py-2" width="12%">Total</th>
                @else
                    @foreach ($documment_columns as $column)
                        <th class="border-left desc text-center py-2" style="text-align: {{ $column->column_align }};"
width="{{ $column->width }}%">
                            {{ $column->name }}</th>
                    @endforeach
                @endif
    ...
    <table class="full-width">
        ...
        @if ($configuration->taxed_igv_visible_doc)
