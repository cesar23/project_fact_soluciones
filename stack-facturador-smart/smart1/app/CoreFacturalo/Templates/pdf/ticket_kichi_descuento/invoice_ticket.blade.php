...
<body class="ticket">
    ...
    <table class="full-width mt-10 mb-10">
        <thead class="">
            <tr>
                ...
                <th class="border-top-bottom desc-9 text-left">P.Desc</th>
                <th class="border-top-bottom desc-9 text-right">Total</th>
            </tr>
        </thead>
        @php
            $total_subtotal = $document->total;
        ...
        <tbody>
            @foreach ($document->items as $row)
                @php
                    // Total original del item (sin descuento)
                    $item_total_original = $row->unit_price * $row->quantity;

                    // Calcular descuento proporcional del item
                    if ($total_subtotal > 0) {
                        $item_discount = ($item_total_original / $total_subtotal) * $document->total_discount;
                    } else {
                        $item_discount = 0;
                    }

                    // Si el tipo de descuento es '02', aplicar IGV al descuento
                    if ($discount_type_id == '02') {
                        $item_discount = $item_discount * 1.18;
                    }

                    // Total del item después de descuento
                    $item_total_final = $item_total_original - $item_discount;
                    if($document->total_discount == 0){
                        $item_total_final = 0;
                    }
                    // Si item_total_final no es 0, ajustar centavos que terminan en .x9 o .x1
                    if ($item_total_final != 0) {
                        $valueStr = number_format($item_total_final, 2, '.', '');
                        $lastDigit = substr($valueStr, -1);
                        
                        if ($lastDigit === '9') {
                            // Si termina en .x9, sumar 0.01
                            $item_total_final = $item_total_final + 0.01;
                        }
                        else if ($lastDigit === '1') {
                            // Si termina en .x1, restar 0.01  
                            $item_total_final = $item_total_final - 0.01;
                        }
                    }
                    if($row->quantity !== 1){
                        $item_total_final = $item_total_final / $row->quantity;
                    }
                @endphp
                <tr>
                    <td style="font-size:11px;" class="text-left align-top" colspan="5">
                ...
                <tr>
                    ...
                    <td style="font-size:11px;" class="text-center align-top">
                        {{ number_format($item_total_final, 2) }}</td>
    ...
    <table class="full-width">
        <tr>
            <td>
            <td class="text-left pt-1" colspan="2">
                    ...
                    <tbody>
                        ...
                        @if ($document->total_free > 0)
                            <tr>
                                <td colspan="4" class="text-right font-bold desc">Op. Gratuitas:
                                    {{ $document->currency_type->symbol }}</td>
                                <td class="text-right font-bold desc">{{ number_format($document->total_free, 2) }}
                                </td>
                            </tr>
                        @endif
                        ...
                        @elseif($document->total_taxed > 0)
                            <tr>
                                <td colspan="4" class="text-right font-bold desc">Op. Gravadas:
                                    {{ $document->currency_type->symbol }}</td>
                                <td class="text-right font-bold desc">{{ number_format($document->total_taxed, 2) }}
                                </td>
                            </tr>
                        @endif
                        ...
                        @if ($document->total_isc > 0)
                            <tr>
                                <td colspan="4" class="text-right font-bold desc">ISC:
                                    {{ $document->currency_type->symbol }}
                                </td>
                                <td class="text-right font-bold desc">{{ number_format($document->total_isc, 2) }}
                                </td>
                            </tr>
                        @endif
                        ...
                        @if ($balance < 0)
                            <tr>
                                <td colspan="4" class="text-right font-bold desc">Vuelto:
                                    {{ $document->currency_type->symbol }}</td>
                                <td class="text-right font-bold desc">{{ number_format(abs($balance), 2, '.', '') }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
        ...
        <tr>
            ...
            <td colspan="2">
                ...
                <div class="desc">
                    ...
                    @endphp
                    <h5 style="margin: 0px;">
                        DESCUENTO: {{ number_format($discount->factor * 100, 0) }}%
                    </h5>
                </div>
            @endif
