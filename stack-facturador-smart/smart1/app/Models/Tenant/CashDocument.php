...
class CashDocument extends ModelTenant
{
    ...
    {
        return $query->select('purchase_id')
            ->whereHas('purchase', function ($q) {
                $q->whereIn('purchases.id', function ($subQuery) {
                    $subQuery->select('pp.purchase_id')
                        ->from('purchase_payments as pp')
                        ->groupBy('pp.purchase_id')
                        ->havingRaw('SUM(pp.payment) = (SELECT total FROM purchases WHERE id = pp.purchase_id)');
                });
            })
            ->where('cash_id', $cash->id)->get()->pluck('purchase_id')->toArray();
    }
