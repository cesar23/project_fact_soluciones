...
class   SaleNote extends ModelTenant
{
    ...
    {
        return $this->hasOne(SupplyPlanDocument::class, 'sale_note_id');
    }
    public function fee()
    {
    ...
    public function sale_note_order_state()
    {
        return $this->hasOne(SaleNoteOrderState::class);
    }
    ...
    public function getCollectionData(Configuration $configuration = null)
    {
        ...
        if(in_array($this->state_type_id, ['01','05'])){
        ...
        $dispatches = $connection->table('dispatches')
            ->select('id','series','number','external_id')
            ->where('reference_sale_note_id', $this->id)
        ...
        if ($dispatch_order) {
            $has_agency_dispatch = $connection->table('agencies_dispatches_table')
        ...
        $credit_days = "-";
        if ($this->fee) {
            $last_fee = $this->fee->last();
            if ($last_fee) {
                $credit_days = $last_fee->date;
            }
        }
        $license_plate = $this->license_plate;
        $modify_sale_unit_price = $this->items->where('modify_sale_unit_price', true)->count() > 0 || $this->total_discount > 0;
        if($configuration->plate_number_config){
            $plante_numbers_description = optional($this->plateNumberDocument)->plateNumber ? optional($this->plateNumberDocument)->plateNumber->description : null;
            $license_plate = $plante_numbers_description;
        }
        $platforms = "";
        if($configuration->show_web_platform_document_sale_note){
            $items = $this->items->pluck('item_id')->unique();
            $platforms = $connection->table('items')
                ->select('web_platform_id')
                ->whereIn('id', $items)
                ->pluck('web_platform_id')
                ->unique();
            $platforms = $connection->table('web_platforms')
                ->select('name')
                ->whereIn('id', $platforms)->pluck('name')->implode(' / ');
        } 
        if($this->no_stock){
            $no_stock_document = $this->no_stock_document;
    ...
    {
        return $this->payments->map(function ($row, $key) {
            return $row->payment_method_type->description;
        });
    }
    ...
    public function plateNumberDocument(){
        return $this->hasOne(PlateNumberDocument::class, 'sale_note_id', 'id');
    }
    ...
    public function getPlatformThroughItems()
    {
        ...
        $connection = DB::connection('tenant');
        return $connection->table('web_platforms')
            ->leftJoin('items', "$web_platform_table_name.id", '=', "$item_table_name.web_platform_id")
    ...
    {
        return $this->hasOne(Dispatch::class, 'reference_sale_note_id');
    }
    /**
    ...
    public function scopeWhereFilterCommissionV2($query, $date_start, $date_end, $establishment_id, $user_type, $user_seller_id, $row_user_id)
    {
    ...
    {
        return $this->hasOne(NoStockDocument::class, 'sale_note_id', 'id');
    }
