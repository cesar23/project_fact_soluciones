...
class Document extends ModelTenant
{
    ...
    public function document_recurrences()
    {
        return $this->hasMany(DocumentRecurrence::class);
    }
    public function auditor_history()
    {
        return $this->hasMany(AuditorHistory::class);
    }
    ...
    public function supply_document()
    {
        return $this->hasOne(SupplyPlanDocument::class);
    }
    ...
    {
        return $this->belongsTo(ChannelDocument::class, 'id', 'document_id');
    }
    ...
    public function dispatch_valid()
    {
        return $this->belongsTo(Dispatch::class, 'dispatch_id')->whereIn('state_type_id', ['01', '03', '05', '07']);
    }
    ...
    public function scopeWhereTypeUser($query, $params = [])
    {
        ...
        return (!$user || $user->type === 'admin' || $user->type === 'superadmin') ? null : $query->where('user_id', $user->id)->orWhere('seller_id', $user->id)->latest();
    ...
    public function reference_guides_valid()
    {
        return $this->hasMany(Dispatch::class, 'reference_document_id', 'id')->whereIn('state_type_id', ['01', '03', '05', '07']);
    }
    ...
    public function scopeWhereFilterCommissionV2($query, $date_start, $date_end, $establishment_id, $user_type, $user_seller_id, $row_user_id, $item_id, $unit_type_id)
    {
        if ($item_id != null && $unit_type_id == null) {
            $query->whereStateTypeAccepted()
                ->whereBetween('date_of_issue', [$date_start, $date_end])
        ...
        //     $q->whereHas('relation_item', function ($q) {
        //         $q->whereNotNull('commission_amount');
        //     });
        // });
    ...
    public function isGeneratedFromExternalRecord()
    {
        ...
        if (!is_null($this->order_note_id) && $configuration->discount_order_note) {
            $generated = true;
        }
    ...
    public function voided_relation()
    {
        return $this->belongsTo(VoidedDocument::class, 'document_id', 'id');
    }

    public function sales_note()
    {
        return $this->hasMany(SaleNote::class, 'id', 'sale_note_id');
    }
    /**
     *
    ...
    public function getPointsBySale()
    {
        ...
        if ($this->isPointSystem()) {
            ...
            $total = $this->total;
            if (isset($point_system_data->point_system_sale_amount)) {
    ...
    public function plateNumberDocument()
    {
        return $this->hasOne(PlateNumberDocument::class, 'document_id', 'id');
    }
    ...
    {
        return $this->document_type_id === DocumentType::CREDIT_NOTE_ID || $this->document_type_id === DocumentType::DEBIT_NOTE_ID;
    }
    ...
    public function isNoAffectedStock()
    {
        if ($this->isNote()) {
            return (bool) $this->note->no_affected_stock;
        }
        return false;
    }
    ...
    public function ajustDocumentFee()
    {
        try {
            $note = Note::where('affected_document_id', $this->id)->first();
            if ($note) {
                $document = $this;
                $creditNote = $note->document->total;
                if ($note->document->state_type_id == '11') {
                    $creditNote = 0;
                }

                $newTotal = $document->total - $creditNote;
                $paid = $document->fee()
                    ->where('is_canceled', true)
                    ->sum('original_amount');
                $balance = max($newTotal - $paid, 0);
                $pendingFees = $document->fee()
                ...
                $originalPendingTotal = $pendingFees->sum('original_amount');
                if ($originalPendingTotal == 0) {
                    return;
                }
                foreach ($pendingFees as $fee) {

                    $proportion = $fee->original_amount / $originalPendingTotal;
                    $fee->original_amount = round($balance * $proportion, 2);
                    $fee->amount = round($balance * $proportion, 2);
                    $fee->save();
                }
            } else {
            }
    ...
    {
        return $this->hasMany(DocumentRelatedToDocuments::class, 'document_id', 'id');
    }
}
