...
class Company extends ModelTenant
{
    ...
    public static function getVcCompany()
    {
        $cache_key = CacheTrait::getCacheKey('vc_company');
        $vc_company = CacheTrait::getCache($cache_key);
        if (!$vc_company) {
            $vc_company = DB::connection('tenant')->table('companies')->select('id','name', 'logo', 'soap_type_id', 'favicon', 'logo_store')->first();
            CacheTrait::storeCache($cache_key, $vc_company);
        }
        return $vc_company;
    }
}
