...
class Quotation extends ModelTenant
{
    ...
    protected $with = ['user', 'soap_type', 'state_type', 'currency_type', 'items', 'payments',"project","fee"];
    ...
    public function technical_service(){
        return $this->hasOne(TechnicalServiceQuotation::class);
    }	
    ...
    {
        return $this->hasOne(QuotationsTechniciansQuotation::class, 'quotation_id');
    }


    public function plateNumberDocument(){
        return $this->hasOne(PlateNumberDocument::class );
    }
    ...
    public function scopeWhereTypeUser($query, $params= [])
    {
        ...
        return ($user->type == 'seller') ? $query->where(function($query) use ($user){
    ...
    public function getCollectionData(Company $company = null, Configuration $configuration = null, $with_items = false){
        ...
        $btn_generate = (($this->hasAcceptedDocuments() || (count($row->sale_notes)  > 0)) && !$configuration->generate_multiple_documents_sale_note)?false:true;
    ...
    public function applyQuotationToCash()
    {
        ...
        $isTotallyPaid = $this->getTotalCanceledByQuotation();

        return ($this->hasStateTypeAccepted() && $this->hasPayments()) && 
               (!$isChanged || ($isChanged && $isTotallyPaid));
    }

    public function getTotalCanceledByQuotation()
    {
    ...
    public function fee()
    {
        return $this->hasMany(QuotationFee::class);
    }
