...
class Purchase extends ModelTenant
{
    ...
    public function  getCollectionData()
    {
        ...
        $alter_company = [];
            ...
            $can_validate = in_array($this->document_type_id, ['01','03','07','08']);
        $exchange_rate_sale = $this->exchange_rate_sale;
        try{
            $total_dollars = number_format($total / $exchange_rate_sale, 2, ".", "");
        }catch(\Exception $e){
            $total_dollars = 0;
        }
        $can_edit = true;
        ...
        $was_transfer = DB::connection('tenant')->table('inventories_transfer')->where('purchase_id', $this->id)->exists();
        
        return [
            'was_dollar' => (bool) $this->was_dollar,
            'was_transfer' => $was_transfer,
            'can_edit' => $can_edit,
            'can_validate' => $can_validate,
            'state_validate' => $this->state_validate,
            'date_validate' => $this->date_validate,
            ...
            'total_dollars'                  => self::NumberFormat($total_dollars),
