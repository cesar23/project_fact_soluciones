...
class Dispatch extends ModelTenant
{
    ...
    {
        return $this->belongsTo(ChannelDocument::class, 'id', 'dispatch_id');
    }
    ...
    {
        return $this->hasOne(InventoryTransfer::class, 'dispatch_id');
    }
    ...
    public function getDownloadExternalCdrAttribute()
    {
        // $type = 'cdr';
        // if($this->website_id){
        //     $company = Company::where('website_id', $this->website_id)->first();
        // }else{
        //     $company = Company::active();

        // }
        // if($company->pse && $company->type_send_pse == 1){
            $type = 'cdr_xml';
        // }
        return route('tenant.download.external_id', ['model' => 'dispatch', 'type' => $type, 'external_id' => $this->external_id]);
    }
    ...
    public function scopeWhereInternal($query)
    {
        return $query->whereIn('state_type_id', ['55', '56']);
    }

    public function scopeWhereNotInternal($query)
    {
        return $query->whereNotIn('state_type_id', ['55', '56']);
    }
    ...
    public function dispatch_orden(){
        return $this->belongsTo(DispatchOrder::class, 'reference_dispatch_order_id');
    }
    /**
     * @return BelongsTo
     */
    ...
    public function getCollectionData()
    {
        ...
        $documents = [];
        if ($this->generate_document && !in_array($this->generate_document->state_type_id, ['09','11'])){
            $documents[] = ['description' => $this->generate_document->number_full];
        }
        if ($this->reference_document && !in_array($this->reference_document->state_type_id, ['09','11'])){
            $documents[] = ['description' => $this->reference_document->number_full];
        }
        $has_auditor_history = $this->auditor_history->count() > 0;
        $btn_pdf = true;
        $btn_send = false;
        $btn_options = false;
        ...
        $is_pse = $company->pse && $company->type_send_pse == 2;
        ...
        if ($this->state_type_id !== '05' && $this->state_type_id !== '11') {
            $btn_edit = true;
        }
        ...
        $channel_name = null;
        if ($this->channel) {
            $channel_name = $this->channel->channel->channel_name;
        }
        $generate_document = $this->generate_document ? !in_array($this->generate_document->state_type_id, ['09','11']) :  false;
        $reference_document = $this->reference_document ? !in_array($this->reference_document->state_type_id, ['09','11']) :  false;
        $items = $this->items->transform(function ($item) {
            $quantity = $item->quantity;
            $unit_price = $item->relation_item->sale_unit_price;
        ...
        return [
            ...
            'auditor_state' => (bool) $this->auditor_state,
            'has_auditor_history' => $has_auditor_history,
            'related_document' => $this->related_document,
            ...
            'btn_generate_document' => $generate_document || $reference_document || !$btn_generate_document ? false : true,
            ...
            'subcontract_company_id' => $this->subcontract_company_id,
            'vehicle_configuration' => $this->vehicle_configuration
        ];
    }
    ...
    public function subcontract_company(){
        return $this->belongsTo(Person::class, 'subcontract_company_id');
    }
