...
class User extends Authenticatable
{
    ...
    protected $fillable = [
        'force_logout_at',
        'show_packers_in_document',
        'show_dispatchers_in_document',
        'show_box_in_document',
        'edit_payment',
        'show_packers_document',
        'show_dispatchers_document',
        'show_box',
        'edit_pos',
        'reopen_pos',
        'delete_pos',
    ...
    protected $casts = [
        'show_packers_in_document' => 'boolean',
        'show_dispatchers_in_document' => 'boolean',
        'show_box_in_document' => 'boolean',
        'show_packers_document' => 'boolean',
        'show_dispatchers_document' => 'boolean',
        'show_box' => 'boolean',
        'edit_pos' => 'boolean',
        'reopen_pos' => 'boolean',
        'delete_pos' => 'boolean',
        'create_order_delivery' => 'boolean',
        'edit_order_delivery' => 'boolean',
        'voided_order_delivery' => 'boolean',
        'voided_cpe' => 'boolean',
        'note_cpe' => 'boolean',
        'voided_sale_note' => 'boolean',
        'pos_lite' => 'boolean',
        'edit_delivery_state' => 'boolean',
        'edit_price_pos' => 'boolean',
        'series_id' => 'int',
    ...
    public static function getAuthUserWithoutRelations()
    {
        return self::withoutGlobalScope('with')->find(auth()->id());
    }

    // Sobreescribir newModelInstance para excluir relaciones en auth()->user()
    protected function newModelInstance($attributes = [])
    {
        $model = parent::newModelInstance($attributes);

        // Si estamos obteniendo el usuario autenticado, no cargar relaciones
        if (auth()->check() && auth()->id() === $model->getKey()) {
    ...
    public static function setCompanyActiveId($id)
    {
        /** @var User::class $user */
        $user = auth()->user();
        $user->company_active_id = $id;
        $user->save();
    }
    ...
    */
    ...
    public function getFilterItemByUser()
    {
        return FilterItemByUser::where('user_id', $this->id)->first();
    }
    ...
    public function scopeGetSellers(
        ...
        if ($configuration->all_sellers == false) {
            if (auth()->user()) {
                $query->where(
                    function ($query) {
                        $query->where(
    ...
    {
        // Invalidar el token API
        $this->api_token = null;
        $this->remember_token = null;
        $this->save();

        // Si usas Laravel Sanctum, también invalidar tokens
        if (method_exists($this, 'tokens')) {
            $this->tokens()->delete();
        }

        // Cerrar sesión si el usuario está autenticado actualmente
        if (auth()->check() && auth()->id() == $this->id) {
            auth()->logout();
        }

        // Limpiar sesión específica del usuario
    ...
    private function clearUserSession()
    {
        ...
        $sessionKey = 'user_session_' . $this->id;
        if (function_exists('cache')) {
            cache()->forget($sessionKey);
        }

        // Si usas Redis o cache personalizado
    ...
    public static function getUserCashId()
    {
        $user = auth()->user() ?? auth('api')->user();
        $user_id = $user->id;
        $user_cash_id = $user->user_cash_id;
        if ($user_cash_id) {
            return $user_cash_id;
        }
        return $user_id;
    ...
    public static function getCashId()
    {
        ...
        if ($user_cash_id) {
            $cash = Cash::where('user_id', $user_cash_id)->where('state', true)->first();
        } else {
    ...
    public function getSeries()
    {
        ...
        $userType = $this->type;
        if ($configuration->seller_establishments_all || $user->type === 'admin') {
            $series = Series::get();
        } else {
            $series = Series::FilterSeries($establishment_id);
            $series = $series->get();
        }
        $series = $series->transform(function ($row) use ($document_id, $series_id, $userType) {
            /** @var Series $row */
            return $row->getCollectionData($document_id, $series_id, $userType);
        })->where('disabled', false)->values();
    ...
    public static function  getSellersToNvCpe($establishment_id = 0, $userId = 0)
    {
        ...
        $query = DB::connection('tenant')->table('users')->select('id', 'name', 'email', 'type', 'establishment_id')->where('is_locked', false);
        if ($configuration->all_sellers || $configuration->seller_establishments_all) {
            return  $query->whereIn('type', ['seller', 'admin'])->orWhere('id', $userId)
                ->get();
        }
    ...
    public function getDataOnlyAuthUser()
    {
        return [
            ...
            'edit_price_pos' => $this->edit_price_pos,
            'id' => $this->id,
            'name' => $this->name,
            'email' => $this->email,
