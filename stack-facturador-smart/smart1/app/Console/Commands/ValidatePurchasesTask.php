...
use App\Models\Tenant\{Company, Configuration, Document, Purchase, StateType};
use Illuminate\Support\Facades\{Storage, Log};
use Carbon\Carbon;
use Modules\Services\Data\ServiceData;

class ValidatePurchasesTask extends Command
{
    protected $signature = 'purchases:validate';
    protected $description = 'Valida compras masivamente con state_type_id = 01';
    ...
    {
        $query = Purchase::whereNull('state_validate')
    ...
    private function formatDocumentLine(Company $company, Purchase $purchase): string
    {
        return implode("|", [
            $purchase->supplier->number,
            $purchase->document_type_id,
            $purchase->series,
            intval($purchase->number),
            $this->formatDate($purchase->date_of_issue),
            $purchase->total
        ]) . "\n";
    }
    ...
    private function updateDocumentState(array $value)
    {
        ...
        if ($estado_comprobante == "NO EXISTE" || $estado_comprobante == "-") {
            $state_type_id = "01";
    ...
    {
        return Purchase::where('series', $value['Serie'])
