...
<head>
    ...
    <title>Recibo Deuda - {{ $data['debt']->serie_receipt }}-{{ $data['debt']->correlative_receipt }}</title>
    <style>
        @page {
            margin: 3mm;
            size: A5 portrait;
...
<body>
            ...
            <tr>
                <td class="recibo-cliente-cell">
                    <div class="recibo-number">
                        RECIBO N°: {{ $data['debt']->serie_receipt }}-{{ str_pad($data['debt']->correlative_receipt, 6, '0', STR_PAD_LEFT) }}
                    </div>
                    ...
                    @endphp
                    <div>JUNTA ADMINISTRADORA DE AGUA POTABLE MAZAMARI - {{  $data['sector']->name ?? '' }} - {{$short}} {{ $data['supplyVia']->name ?? '' }}</div>
                    <div>
                        @if($data['debt']->month && $data['debt']->year)
                            PERÍODO: {{ $data['monthName'] }} {{ $data['debt']->year }}
                        @else
                            FECHA: {{ $data['debt']->generation_date ? $data['debt']->generation_date->format('d/m/Y') : date('d/m/Y') }}
                        @endif
                    </div>
                ...
                <td class="logo-cell">
                    <img src="{{ resource_path('views/tenant/supplies/documents/logo.png') }}" class="logo" alt="Logo">
                </td>
                    ...
                    <table class="tabla-importes" style="margin: 0;">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="right">{{ number_format($data['debt']->amount, 2) }}</td>
                            </tr>
                            @php
                                $totalRows = 10;
                                $contentRows = 2;
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td></td>
                                <td class="left bold">S/ {{ number_format($data['debt']->amount, 2) }}</td>
                            </tr>
                        </tfoot>
                        </tbody>
                    </table>
                    ...
                    <table class="tabla-fechas">
                        <tr>
                            <td class="info-box">
                                ...
                                <div>{{ $data['debt']->generation_date ? $data['debt']->generation_date->format('d-m-Y') : $data['date'] }}</div>
                            </td>
                        </tr>
                        <tr>
                            <td class="info-box">
                        ...
                        <tr>
                            <td class="info-box">
                                ...
                                <div class="vencido">{{ $data['debt']->due_date ? $data['debt']->due_date->format('d-m-Y') : 'N/A' }}</div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                        ...
                        <tr>
                            <td class="text-center">
                                <div style="text-align: center; display: flex; justify-content: center; align-items: center;">
                                    @php
                                        $isOverdue = $data['debt']->due_date && $data['debt']->due_date->isPast() && !$data['debt']->active;
                                        $isPaid = $data['debt']->active;
                                    @endphp
                                    @if ($isPaid)
                                        <img src="{{ resource_path('views/tenant/supplies/documents/happy.png') }}" class="emoji" alt="Pagado">
                                    @elseif ($isOverdue)
                                        <img src="{{ resource_path('views/tenant/supplies/documents/sad.png') }}" class="emoji" alt="Vencido">
                                    @else
                                        <img src="{{ resource_path('views/tenant/supplies/documents/happy.png') }}" class="emoji" alt="Al día">
                                    @endif
                                </div>
        ...
        <table class="footer-messages-table">
            <tr>
                ...
                <td class="message-right">
                    "Si sufres cualquier forma de violencia familiar, no la justifiques. Da el primer paso y busca apoyo."
                    <img src="{{ resource_path('views/tenant/supplies/documents/linea.png') }}" class="line" alt="Línea">
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="resumen-container">
        <table class="header-table">
            <tr>
                <td class="recibo-cliente-cell">
                    ...
                    @endphp
                    <div><strong>CLIENTE:</strong> <span>{{ $clientNameResumen }}</span></div>
                    <div><strong>DIRECCIÓN:</strong> {{ strtoupper($data['supply']->optional_address ?? ($data['sector']->name ?? '')) }}</div>
                    <div><strong>CONCEPTO:</strong> {{ $data['description'] }}</div>
                    @if($data['debt']->month && $data['debt']->year)
                        <div><strong>PERÍODO:</strong> {{ $data['monthName'] }} {{ $data['debt']->year }}</div>
                    @endif
                </td>
                <td class="logo-cell">
                    <img src="{{ resource_path('views/tenant/supplies/documents/logo.png') }}" class="logo" alt="Logo">
        ...
        <table width="100%">
            <tr>
                <td width="100%">
                    <div style="text-align: center; width: 40%; border: 1px solid #333; padding: 5px; margin-left: auto;">
                        <div class="info-box-title">FECHA VENCIMIENTO:</div>
                        <div class="vencido">{{ $data['debt']->due_date ? $data['debt']->due_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                </td>
        ...
        <div class="resumen-total">
            @if($data['debt']->active)
                PAGADO S/: {{ number_format($data['debt']->amount, 2) }}
            @else
                TOTAL A PAGAR S/: {{ number_format($data['debt']->amount, 2) }}
            @endif
        </div>
    </div>
</body>
