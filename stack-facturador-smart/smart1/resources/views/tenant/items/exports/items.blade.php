                    ...
                    <tbody>
                        @foreach ($records as $key => $value)
                            ...
                            <tr>
                                ...
                                @endfor
                                @foreach ($warehouses as $warehouse)
                                    <td>{{ optional($value->warehousePrices->where('warehouse_id', $warehouse['id'])->first())->price ?? 0 }}
                                    </td>
                                @endforeach
                            </tr>
                            @if (count($value->attribute) > 0)
                                <tr>
                                    ...
                                    <td style="font-weight: bold;">Año de Modelo</td>
                                    <td style="font-weight: bold;">Color</td>
                                    <td style="font-weight: bold;">Motor</td>
                                    <td style="font-weight: bold;">Modelo</td>
                                    <td style="font-weight: bold;">Serie / Chasis</td>
                                </tr>
                                @foreach ($value->attribute as $attribute)
                                    <tr>
                                        <td></td>
                                        <td>{{ $attribute['attribute'] ?? '' }}</td>
                                        <td>{{ $attribute['attribute5'] ?? '' }}</td>
                                        <td>{{ $attribute['attribute3'] ?? '' }}</td>
                                        <td>{{ $attribute['attribute4'] ?? '' }}</td>
                                        <td>{{ $attribute['attribute2'] ?? '' }}</td>
                                        <td>{{ $attribute['chassis'] ?? '' }}</td>
                                    </tr>
                                @endforeach
                            @endif
                        @endforeach
