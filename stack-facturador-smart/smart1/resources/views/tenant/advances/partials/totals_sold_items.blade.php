    ...
    <tbody>
        ...
        @php
            ...
            $table_sold_items = $sold_items->map(function ($row, $key) {
                ...
                $quantity = $row->reduce(function ($carry, $g_item) {
                    $item = $g_item['item'];
                    $g_quantity = $g_item['quantity'];
                    $qty = 0;
                    $factor_presentation = 1;
                    if (isset($item->presentation)) {
                        $presentation = $item->presentation;
                        // if($presentation != null || )
                        //si presentacion no es un objeto, entonces es un array
                        if (is_object($presentation)) {
                            $factor_presentation = $presentation->quantity_unit;
                        }
                    }
                    $qty = $g_quantity * $factor_presentation;
                    return $carry + $qty;
                }, 0);
                $total = $row->sum('total');
        ...
        @foreach ($table_sold_items as $item)
            <tr>
                ...
                <td class="celda" style="text-align: right">
                    {{ ReportHelper::setNumber($item['unit_value'], 2, ',', '.') }}</td>
                <td class="celda" style="text-align: right">{{ ReportHelper::setNumber($item['total'], 2, ',', '.') }}
                </td>
            </tr>
        @endforeach
