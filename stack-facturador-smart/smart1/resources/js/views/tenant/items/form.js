...
import ItemAttributes from "./partials/item_attributes.vue";
import {
    ItemOptionDescription,
    ItemSlotTooltip,
} from "../../../helpers/modal_item";
import FormEcommerce from "./form_ecommerce.vue";
export default {
    mixins: [functions],
    props: [
        "configuration",
    ...
    data() {
        return {
            ...
            // configuration: {},
            // Arrays para selects - inicializados vacíos
            unit_types: [],
            currency_types: [],
            system_isc_types: [],
            ...
            loadingData: false,
            editRowAttribute: false,
            dataAttribute: [],
            showDialogAttribute: false,
            warehouseUserId: null,
            isWeaponTracking: false,
        };
    },
    watch: {
        showDialog: {
            handler(newVal) {
                if (newVal) {
                    ...
                    this.$nextTick(async () => {
                        await this.loadStaticDataIfNeeded(); // Cargar datos primero
                        this.create(); // Luego inicializar
                        this.loadingData = false; // Desactivar loading
                    });
                }
            },
            immediate: true,
    ...
    async created() {
        ...
        // Solo inicializaciones básicas - los datos estáticos se cargan manualmente
    ...
    methods: {
        addRowAttributes(attributes) {
            this.form.attributes_list = attributes;
       },
        clickLotAttribute() {
            this.editRowAttribute = this.recordId != null ? true : false;
            this.dataAttribute = this.form.item_attribute;
            this.showDialogAttribute = true;
        },
        async loadStaticDataIfNeeded() {
            if (this.staticDataLoaded) {
                return; // Ya están cargados los datos estáticos
            }
            ...
            await this.$http
                ...
                .then((response) => {
                    ...
                    this.isWeaponTracking = data.is_weapon_tracking;
                    ...
                    this.warehouseUserId = data.warehouse_user_id;
                    if (this.warehouseUserId) {
                        this.form.warehouse_id = this.warehouseUserId;
                    }
        ...
        resetForm() {
            ...
            if (this.recordId) {
                ...
                this.activeName = "ecommerce";
                this.previousTab = "ecommerce";
            }
        ...
        changeTab() {
            ...
            // Solo copiar valores si la pestaña anterior o actual es "ecommerce"
            const shouldCopyValues =
                this.previousTab === "ecommerce" ||
                this.activeName === "ecommerce";

            if (shouldCopyValues) {
                if (this.activeName !== "ecommerce") {
                    let formEcommerce = this.$refs.formEcommerce;
                    if (!formEcommerce || !formEcommerce.form) {
                        return;
                    }
                    let formEcommerceData = formEcommerce.form;

                    // Propiedades compartidas que se copian del formulario ecommerce al formulario principal
                    ...
                    // Solo copiar si el formulario ecommerce tiene datos válidos
                    const hasEcommerceData = sharedProperties.some((prop) => {
                        const value = formEcommerceData[prop];
                        return (
                            value !== undefined &&
                            value !== null &&
                            value !== ""
                    ...
                    if (hasEcommerceData) {
                        // Copiar propiedades del formulario ecommerce al formulario principal
                        sharedProperties.forEach((prop) => {
                            if (
                                formEcommerceData[prop] !== undefined &&
                                formEcommerceData[prop] !== null &&
                                formEcommerceData[prop] !== ""
                            ) {
                                this.form[prop] = formEcommerceData[prop];
                            }
                        });
                    }
                } else {
                    ...
                    // Propiedades compartidas que se copian del formulario principal al ecommerce
                    const sharedProperties = [
                        "description",
                        "name",
                        "second_name",
                    ...
                    // Solo copiar si el formulario principal tiene datos válidos
                    const hasMainFormData = sharedProperties.some((prop) => {
                    ...
                    if (hasMainFormData) {
                        // Copiar propiedades del formulario principal al formulario ecommerce
                        sharedProperties.forEach((prop) => {
                            if (
                                ...
                                formEcommerce.form[prop] = this.form[prop];
                            }
                        });
                    }
                }
            ...
            // Guardar la pestaña actual como anterior para la próxima navegación
            this.previousTab = this.activeName;
        },
        reloadDataItemsEcommerce(id) {
        ...
        validateUnitTypeRanges() {
            ...
            for (let i = 0; i < this.form.item_unit_types.length; i++) {
                ...
                if (
                    price_default_value < 0.07 &&
                    !this.configuration.price_item_007
                ) {
                    this.$message.error(
                        `Línea ${
                            i + 1
        ...
        async loadLabelOptions() {
            try {
                ...
                console.error(
                    "Error al cargar las opciones de etiquetas:",
                    error
                );
            }
        },
        ...
        openFileInputIngredient() {
            this.$refs.fileInputIngredient.click();
        },
        handleFileChangeIngredient(event) {
            const file = event.target.files[0];

            const allowedTypes = ["image/jpeg", "image/png", "image/gif"];
            if (file && allowedTypes.includes(file.type)) {
                this.form_ingredient.file = file;
                const reader = new FileReader();
                reader.onload = () => {
                    this.selectedImageIngredient = reader.result;
                };
                reader.readAsDataURL(file);
            } else {
                this.selectedImageIngredient = null;
                this.$message.error(
                    "Por favor, seleccione un archivo de imagen válido (JPEG, PNG o GIF)."
                );
            }
        ...
        openFileInputLine() {
            this.$refs.fileInputLine.click();
        },
        handleFileChangeLine(event) {
            const file = event.target.files[0];

            const allowedTypes = ["image/jpeg", "image/png", "image/gif"];
            if (file && allowedTypes.includes(file.type)) {
                this.form_line.file = file;
                const reader = new FileReader();
                reader.onload = () => {
                    this.selectedImageLine = reader.result;
                };
                reader.readAsDataURL(file);
            } else {
                this.selectedImageLine = null;
                this.$message.error(
                    "Por favor, seleccione un archivo de imagen válido (JPEG, PNG o GIF)."
                );
            }
        ...
        setDefaultConfiguration() {
            ...
            if (!this.recordId) {
                this.form.has_igv = this.config.include_igv;
                this.form.purchase_has_igv = this.config.include_igv;
            }
            this.item_complements = this.config.item_complements;
        ...
        async reloadTables() {
            await this.$http
                ...
                .then((response) => {
                    ...
                    this.warehouseUserId = response.data.warehouse_user_id;
                    if (this.warehouseUserId) {
                        this.form.warehouse_id = this.warehouseUserId;
                    }
        ...
        checkUnitValue() {
            if (this.configuration && !this.configuration.price_item_007 && !this.isWeaponTracking) {
        ...
        close() {
            ...
            this.$nextTick(() => {
                if (
                    this.$refs.formEcommerce &&
                    this.$refs.formEcommerce.resetForm
                ) {
                    this.$refs.formEcommerce.resetForm();
                }
            });
        ...
        saveIngredient() {
            this.form_ingredient.add = false;
            this.form_ingredient.image = this.selectedImageIngredient;
            const formData = new FormData();
            formData.append("image", this.form_ingredient.file);
            formData.append("name", this.form_ingredient.name);

            this.$http
                .post(`/ingredient-attributes`, formData, {
                    headers: {
                        "Content-Type": "multipart/form-data",
                    },
                })
                .then((response) => {
                    if (response.data.success) {
                        ...
                        this.form_ingredient.name = null;
                        this.selectedImageIngredient = null;
                    } else {
                        this.$message.error("No se guardaron los cambios");
                    }
        ...
        saveLine() {
            this.form_line.add = false;
            this.form_line.image = this.selectedImageLine;
            const formData = new FormData();
            formData.append("image", this.form_line.file);
            formData.append("name", this.form_line.name);

            this.$http
                .post(`/line-attributes`, formData, {
                    headers: {
                        "Content-Type": "multipart/form-data",
                    },
                })
