function calculateRowItem(row_old, currency_type_id_new, exchange_rate_sale, pigv,isEdit) {
    ...
    let unit_price = parseFloat(row_old.item.unit_price || row_old.unit_price)
...
function calculateRowItemQuotation(row_old, currency_type_id_new, exchange_rate_sale, pigv,isEdit) {

    let old_total_value = row_old.unit_value;
    console.log("old_total_value",old_total_value);
    let currency_type_id_old = row_old.item.currency_type_id
    ...
    let percentage_igv = pigv * 100
    let unit_value = row.unit_price
    if(row_old.total_discount > 0 || row_old.total_charge > 0){
        unit_value = old_total_value;
    }
    ...
    if(row_old.total_discount > 0 || row_old.total_charge > 0){
        row.discounts = row_old.discounts;
        row.charges = row_old.charges;
        row.total_discount = row_old.total_discount;
        row.total_charge = row_old.total_charge;
    }else{
        if (row.discounts && typeof row.discounts === 'object') {
            row.discounts = Object.values(row.discounts)
        }
    ...
    let total_discount = discount_base + discount_no_base
    if(row_old.total_discount > 0 || row_old.total_charge > 0){ 
        total_discount = row_old.total_discount;
    }
    console.log("total_discount",total_discount);
    let total_charge = charge_base + charge_no_base
    if(row_old.total_discount > 0 || row_old.total_charge > 0){
        total_charge = row_old.total_charge;
    }
    ...
    if(row_old.total_discount > 0 || row_old.total_charge > 0){
        row.unit_value = old_total_value;
    }

    console.log("row",row.unit_price)
    console.log("row_total",row.total)
    return row
}

function calculateRowItemNote(row_old, currency_type_id_new, exchange_rate_sale, pigv,isEdit) {
    // console.log(currency_type_id_new, exchange_rate_sale)
...
export {
    calculateRowItem, calculateRowItemNote,
    calculateRowItemQuotation,
    getUniqueArray, showNamePdfOfDescription,
